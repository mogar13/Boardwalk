...
- **The house will bank a lone player, and the difference from v1's faucet is one inequality.** ✅
  Live (slice 5 of [plans/UNO_HOUSE_RULES.md](plans/UNO_HOUSE_RULES.md)). One human against bots
...
- **A HOUSE RULE IS A CREATE-TIME ROOM PARAMETER, not a `manifest.option`.** ✅ Live (slices 1–3 of
  [plans/UNO_HOUSE_RULES.md](plans/UNO_HOUSE_RULES.md)) — **every rule still ships defaulted OFF, so
...
| The room referee arbitrates seats, and a forged uid cannot claim one | `boardwalk-api/tests/rooms.test.ts` (38, the store/seat logic) + `gateway.test.ts` (26, driven over a REAL socket) — handshake auth, host-only gating, monotonic `seq`, owner-only private hands, author-pinned chat, disconnect→seat-release |
...
| Every guard above actually fires | `tests/lint-rules.test.ts` (48 — the two Phase-6 rules proved **twice**, once per games tree, falsified by dropping `packages/game-logic/src/games` from `GAMES_DIRS` and watching exactly the three new cases go red), `tests/file-size-guard.test.ts` (8), `tests/credentials.test.ts` (25), `tests/firebase-config.test.ts` (8) |

| Not yet enforced | Lands in |
|---|---|
| **UNO's house-banked table (slice 5) is DEPLOYED to the Pi** | ⛔ **OWED.** It is a money change in `boardwalk-api` — `startMatch` pins the tier and `settleMatch` pays a pot the house funded — so unlike stacking it cannot reach prod through the shared package alone, and **the Pi goes FIRST**. **The window is a UI that lies, and it costs nobody a chip**: a new client creating an AI table at a stake sends `anteCents` to an old referee whose `potFor` still floors a lone player to zero, so the lobby says "the house banks the pot", the board draws no pot, and *nothing is charged and nothing is paid*. That is the same benign-but-wrong shape slices 1–2 shipped in, and it is exactly why the order is what it is. The reverse direction is fully inert: an old client has no ante picker in AI mode, so a deployed referee sees no house table at all. What to verify from the ARTIFACT and not from an exit code, once it runs: the path-stripped `md5sum` over both `src` trees byte-identical to a clean worktree at `origin/main`; `packages/game-logic/dist/games/uno/logic/pot.js` carrying `housePayout`/`rankedPayees`/`maxRoundPayout` and `dist/domain/uno.js` naming `HOUSE_TABLE_LEVEL`; the running PID postdating the build with `cwd` in the rsync'd tree; the boot sweep priced BEFORE restarting (`SELECT id, pot_cents FROM uno_matches WHERE settled = 0` — a restart voids and refunds every live round); and the ledger byte-identical either side |
