...
`text-3xl` in a text token rather than `text-2xl` in a border one. **The active-colour pill is gone**: it said RED under a red card
beside a red-tinted border, and the one case it was load-bearing for (a wild, whose chosen colour
lives nowhere else) is answered by painting the discard's HALO from the active colour instead.
