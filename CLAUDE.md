...
- **The one game that can win money does not deal its own cards.** ✅ Live (Phase D, deployed
  ...
  **`BlackjackRepo` — the ROOM-LESS hand's seam, `deal`/`move`, with an offline twin — is GONE**, in
  ...
  is a road back to somewhere, and there is nowhere for that one to go. **The `POST /blackjack/deal`
  and `/move` ROUTES are deliberately still on the Pi** — a cached page still holding the old bundle
  would 404 on them the moment they were removed, so they come out on the next hand-deploy rather
  than in the commit that stops calling them. That is the cutover order REVERSED from the usual
  rule, and deliberately: the Pi goes first when a client would otherwise reach for something that
  does not exist yet, and last when it is the client that is being taken away.
