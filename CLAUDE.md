...
- **An offline result is banked against a SERVER-SIGNED nonce, and the batch is the bound.** ✅ Live
  ...
  Design and the drive evidence: [plans/OFFLINE_HARDENING.md](plans/OFFLINE_HARDENING.md).
...
| The room referee arbitrates seats, and a forged uid cannot claim one | `boardwalk-api/tests/rooms.test.ts` (19, the store/seat logic) + `gateway.test.ts` (7, driven over a REAL socket) — handshake auth, host-only gating, monotonic `seq`, owner-only private hands, author-pinned chat, disconnect→seat-release |
...
| **Offline hardening: server DEPLOYED, enforcement OFF pending the client** | The Pi runs the ticket code as of 2026-07-18 (`ticket_devices` created, 231/231 green on the device, ledger byte-identical) but `TICKET_SECRET` is deliberately **unset**, so `/health` reports `tickets: "off"` and `/settle` still accepts client-minted nonces. That is the documented fail-open, and here it is the cutover switch: **setting the secret is Phase 3, AFTER the client ships.** Setting it early 409s every settle from the deployed frontend — which happened, for ~2 minutes, because this row's first draft said "the Pi goes first" without saying that the secret is not part of the server deploy. Rollback is renaming the env key and restarting, no rebuild. Three-phase procedure in [plans/OFFLINE_HARDENING.md](plans/OFFLINE_HARDENING.md#deploy-order--three-phases-and-the-secret-goes-last) |
