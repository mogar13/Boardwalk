...
nothing else.** It declares `modes: ['ai', 'online']` and `seats: { min: 2, max: 4 }`, where it used
...
|---|---|
| **The room-less blackjack SERVER half is REMOVED and the Pi is DEPLOYED** | ✅ **2026-08-09, verified from the artifact — and this is the ordering rule RUN BACKWARDS ON PURPOSE, the first time that has happened here.** Every other row in this table deploys the Pi FIRST, because a new client must never reach for a frame the referee has not learned. **Removal is the mirror case: the client half shipped in #87, so the Pi goes LAST** — taking `POST /blackjack/deal|move` away before the cached bundle that calls them could no longer be live would 404 a real player mid-hand. So PR #89 merged, sat, and only then did the box change. **The elapsed-time test was replaced by an EVIDENCE test, which is the part worth copying**: only **1h 6m** had passed since #87, which does not clear "no stale tab plausibly survives" by any honest reading — so rather than guess, `blackjack_hands WHERE settled = 0` was asked, and it was **0**, with **0** open wagers naming a hand. No tab can be mid-hand on a path with no live hand on it, so the removal strands nothing *whatever the clock says*. A question about a duration became a question about a row. **The currency check earned its keep twice.** Per-FILE hashes said the Pi differed by *exactly* this change and nothing else — `boardwalk-api/src` 36 files against the branch's 34 (the two extra being the two deleted files, `app.ts` and `db/schema.ts` modified) and `packages/game-logic/src` 37 against 36 (the extra being `logic/view.ts`) — which is the one-step answer a rolled-up digest cannot give: it proves both that the box was behind by this alone and that **PR #86's outage-hardening work, in flight in a concurrent worktree, was not about to ride along or be clobbered**. That mattered concretely: #86 installed a mount watchdog on this very box, and it was checked BEFORE the rsync rather than after — it lives at `/usr/local/sbin/boardwalk-mount-watchdog.sh` with its units in `/etc/systemd/system/`, and there is no `deploy/` directory inside the rsync target at all, so `--delete` could not reach it. **THE CATCH THIS ROW EXISTS FOR: `npm run build` EXITED 0 AND LEFT THE DELETED CODE ON THE BOX.** `tsc` compiles what exists; it does not delete outputs for sources that stopped existing. So after a green build from byte-identical source, `dist/routes/blackjack.js`, `dist/domain/blackjack.js` and the shared `dist/…/logic/view.js` were all still there — dated **2026-08-08 16:46**, the aiPace deploy, against the fresh build's **19:34**. It was inert (`app.js` named `blackjackRouter` 0 times, and the only thing requiring `domain/blackjack` was `routes/blackjack.js` itself — a disconnected island), so nothing would have misbehaved and the deploy would have been recorded as complete. It would also have left the road standing, which is the entire point of the change. `rm -rf dist` on both trees plus a rebuild took **36 → 34** and **37 → 36** files. *This is why the rule is read the artifact, not the exit code — here the exit code was 0 and it was still wrong.* Evidence, none of it an exit code: `src` byte-identical to the branch after the two rsyncs (**34/34** api, **36/36** shared); both deleted `dist` modules and `view.d.ts` **gone**, `app.js` naming `blackjackRouter` **0** times, the shared barrel naming `logic/view` **0** times and **no** dist file mentioning `routes/blackjack` at all, while `blackjackTable.js`, `blackjackDealer.js`, `sweepAbandonedRounds` and the shared `table.js`/`blackjack.js` are all present and `node_modules/@boardwalk/game-logic` resolves to `/home/mogar13/packages/game-logic`; **396/399 on the device**, the 3 the expected `../CLAUDE.md` ENOENT — *named rather than assumed*, by reading the failure text AND `ls`-ing `/home/mogar13/CLAUDE.md` to confirm it genuinely is not there beside a standalone directory; and the mtimes running **src (19:32:12) → dist (19:36:29 api, 19:36:57 shared) → PID (19:38:10)** with `cwd` in the rsync'd tree, `MainPID` 3842968 → **3899218**, `NRestarts: 0`. **The boot sweep was priced BEFORE restarting**: 0 UNO, 0 Liar's Dice, 2 blackjack rounds holding **0 cents** staked, 0 open wagers — and the journal says `[blackjack] voided 2 abandoned round(s), refunded 0 cents`, the predicted cost paid in full, this time with no drift between the pricing and the restart. A **fresh backup was taken first** (`boardwalk-20260810T013320Z.db`, verified 5 profiles / 69 rows / $21,226.50). Ledger byte-identical either side — 5 profiles, 69 rows, $21,226.50, every per-uid sum unchanged, still exactly **one** `void` row (the #88 one; no new ones, because nothing was owed), `integrity_check ok` — and **`blackjack_hands` still carries its 15 rows**, kept as history on purpose. `/health` on the Funnel answers `{"ok":true,"db":"up","tickets":"on"}` with CORS 204 for the Pages origin. **The obvious liveness probe is INCONCLUSIVE and is written down so nobody repeats it**, exactly as the refill deploy found: `POST /blackjack/deal` → 401 looks like proof the route is gone, but `POST /bogusroute-does-not-exist` → 401 too, because auth runs before routing. The artifact is the only proof. **And the restart stopped CLEANLY** (SIGTERM, `Deactivated successfully`) — the `Failed with result 'timeout'` line in the journal buffer is from an EARLIER restart, not this one, which is worth saying because this table has taught everyone to expect the lie |
| **A blackjack TABLE still deals with the room-less server half gone** | unguarded by construction — the removed routes are not the ones a table uses, and the whole point was to PROVE that rather than assume it. Driven 2026-08-09 on the **WS referee** (emulator + a locally-run `boardwalk-api` built from the deployed commit + Vite, on ports nobody else was on — the aiPace run took down another session's dev server by killing a PID it found by port), with the API gate relaxed for the run in a THROWAWAY worktree that was deleted afterwards, so the relaxation could not reach the repo. **The entrance offers exactly `SOLO / AI` and `PLAY ONLINE`** — two ways in, not #87's three, visible. An AI table created at `/play/blackjack?table=FY7S&mode=ai`, a $6 stake placed, and the round dealt, played out and **SETTLED** ("DEALER TAKES IT. $-6.00 this round." on the `<GameResult>` modal) with **11 card images on the felt and 0 broken**. **Zero console errors**; the only 4xx the documented sign-up `404 GET /profile` (a new account's first read before `PUT /profile` creates the row). **The half that actually settles the question is server-side**, because card images only prove something drew: the drive's own SQLite carries `blackjack_rounds` **1**, `blackjack_players` **1** and `blackjack_hands` **0**, with the wager named by `match_id` and `hand_id` NULL. The table dealt entirely through the live path and never once touched the one that was removed — which is the claim, stated as a row rather than as a screenshot. *One thing the drive cost, worth writing down: the sign-in page opens on SIGN IN and the sign-up form is behind "Create one instead", and filling inputs BY INDEX puts the password into the OPTIONAL email field, so the submit silently never happens and the run reads as "the hub never loaded".* |
| **A $5 stake was VOIDED BY HAND on prod, and no deploy was run** | ✅ **2026-08-09.** Two facts, and the first is the unusual one: **deleting the room-less blackjack hand STRANDED an open wager.** `wagers` row 11 (`hand_id: 6`, $5.00, opened 01:15) belonged to a hand nothing shipped can finish any more, and the boot sweep does not reach it — `sweepAbandonedRounds` covers `blackjack_rounds`, never `blackjack_hands`, checked in the built artifact rather than assumed. So it would have sat open forever. It was voided and refunded in ONE guarded transaction matching `voidRound`'s own shape (`UPDATE ... WHERE settled = 0`, then `appendLedger(uid, 'blackjack', +500, 'void')`), against a **fresh backup taken first**. Read back: 53 → **54 rows**, grand total 2,285,000 → **2,285,500**, that uid 285,000 → **285,500**, open wagers 1 → **0**, hand 6 `settled` 0 → **1**, exactly ONE `void` row, every other uid's sum unchanged, `integrity_check ok`. **The hand was holding 21 against a dealer 20 and was NOT played out** — deciding a result by hand on the prod ledger is the exact thing this architecture exists to make unspellable, and void-and-refund is what the sweep does for every other abandoned game. **NO deploy and NO restart**: per-FILE hashes said the Pi was byte-identical to `origin/main` at `4c69c56` on both trees (**36/36** api, **37/37** shared), because PR #87 touched neither — so a restart would have bought nothing and cost something, as a live blackjack round was in progress on table `D7C3` and the boot sweep voids those. `MainPID` 3842968 unchanged throughout, `/health` `{"ok":true,"db":"up","tickets":"on"}`. The two restarts logged at 15:52 and 15:57 that day were somebody else's manual stop/start cycles, not a crash (`NRestarts: 0`) — the documented `Failed with result 'timeout'` lie on the STOP, again. **What was owed here is now PAID — `POST /blackjack/deal|move`, `domain/blackjack.ts` and the shared `viewOf`/`HandView` came off the box the same day; see the row above.** **`blackjack_hands` and `wagers.hand_id` did NOT go with them** — an earlier reading of this row called those 15 rows "dead" and due to be dropped, which was wrong: dropping them means rewriting live tables to reclaim nothing, and the ledger is the money record either way. They stay as history |
