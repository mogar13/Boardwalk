...
- **Money moves as an INTENT, and the server prices it.** ✅ Live (BACKEND_PLAN.md Phase B, code
  ...
  **None of those types has a field for a balance, a price, an XP amount, a stat count, a clock, a
  ...
  `VITE_API_ECONOMY=0`.
- **A bankrupt player has a way back, and the way back cannot be a faucet.** ✅ Live in the tree,
  **not yet deployed** (the Pi goes first, by hand). V1_FEATURE_GAPS.md #10 called the missing
...
- **An offline result is banked against a SERVER-SIGNED nonce, and the batch is the bound.** ✅ Live
  in the tree, **not yet deployed** (the Pi goes first, by hand). The locked Phase-B decision — offline
...
- **A pre-game option is manifest DATA, and the OS draws the control.** ✅ Live —
  ...
  next to the reducer it feeds, pure). Solitaire's draw-1/draw-3 is the first and only caller — it
  had already hand-rolled the picker into its header, which is the shape v1 repeated across ~20
  games. Only `type: 'select'` exists: v1's colour swatch has no caller here, and a control type
...
| A declared pre-game option is well-formed, and resolves to a value the game offers | `tests/game-options.test.ts` (8) — `resolveOptionValues` turning nothing/partial/unoffered/wrong-typed/foreign-keyed input into a complete valid set, `setOptionValue` refusing an unknown id or unoffered value **by identity** (a no-op that does not re-render), and the DECLARATION half over the real registry: unique option ids, unique choice values, and **the default is one of the choices** — the failure that typechecks, throws nothing, and renders a control with nothing selected (falsified by re-defaulting Solitaire's `draw` to `'2'`) |
...
| **The Pi is BEHIND `main`, and the queue is now two deep** | unguarded — **owed, by hand**. `main` already carried Liar's Dice's server half undeployed; the bankrupt refill's `POST /refill` joins it. Blast radius if the frontend merges first, stated so it is a decision and not a surprise: the refill card renders **only** below `REFILL_FLOOR_CENTS`, so on an account that is not broke it does not exist, and a click against a Pi that lacks the route gets a non-2xx that is not a 409 — which `httpEconomyRepo` throws on and `useRefill` turns into "Could not top up — try again." No money moves and nothing crashes. The rank names are pure frontend and need no deploy at all. Procedure: [BACKEND_PLAN.md](plans/done/BACKEND_PLAN.md#the-deploy-delta-phase-d--done-and-what-it-turned-out-to-be) — and **rsync `packages/game-logic` as a sibling first**, because `refillGrantFor` is new shared code the API imports |
