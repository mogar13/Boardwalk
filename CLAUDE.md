...
|---|---|
| **UNO's ranked places (slice 3) is NOT yet on the Pi** | unguarded, and **owed** — the ordering rule, again, and this time it reaches MONEY. The rotation rides to the referee for free through the shared package, but `settleMatch` is server code: until `packages/game-logic` and `boardwalk-api` are rsync'd and the service restarted, a table created with "Play for places" ticked is dealt by a referee that ends the round on the first player out and pays the whole pot to them. **Both deploy directions degrade benignly, by construction and tested rather than hoped for.** New client / old referee: no `finished` on the wire, `placesOf` reads it as an empty podium, every board renders exactly what it renders today — the toggle is a control that does nothing, which is a UI that lies, which is why the Pi goes FIRST. Old client / new referee: `UnoState.winner` is deliberately kept on the wire (derived, `roundOver ? finished[0] : -1`) precisely so a client that predates places still knows the round ended and still draws its result panel. Nothing in this repo can prove it landed — `md5sum` the `src` trees against a clean worktree at `origin/main`, check `dist/` for `seatAfterLive`/`potSplit`, and check the running PID postdates the build ([pi-deploy-procedure]). **Check the boot sweep BEFORE restarting**: it voids and refunds every live match, so read `pot_cents` on the unsettled rows first |
