...
- **A HOUSE RULE IS A CREATE-TIME ROOM PARAMETER, not a `manifest.option`.** ✅ Live (slices 1–2 of
  ...
  nothing observable; **slice 2 (STACKING) is the first rule the reducer enforces**, and ranked
  places lands on top of the same seam. The options seam is the declared home for a pre-game choice and is the wrong one
...
|---|---|
| **UNO's house rules (slices 1–2) are NOT yet on the Pi** | unguarded, and **owed** — this is the ordering rule, not a nicety. The rulebook is shared code the referee runs from its own built copy, so until `packages/game-logic` is rsync'd and the service restarted, a table created with Stacking ticked is dealt by a referee that has never heard of it. **The degradation is benign by construction and was built that way rather than hoped for**: an old referee sends no `houseRules` and no `pendingDraw`, `drawDebt` reads both as nothing owed, and every client plays the ordinary game in agreement with the dealer — no crash, no divergence, just a toggle that does nothing. That is a UI that lies, which is why the Pi still goes FIRST. Nothing in this repo can prove it landed; check `dist/` for `pendingDraw`/`answersStack` and the running PID against the build mtimes, per [pi-deploy-procedure] |
