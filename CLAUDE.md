...
- **A HOUSE RULE IS A CREATE-TIME ROOM PARAMETER, not a `manifest.option`.** ✅ Live (slice 1 of
  [plans/UNO_HOUSE_RULES.md](plans/UNO_HOUSE_RULES.md)) — and **every rule ships defaulted OFF, so
  this changed nothing observable: it is the seam, not the rules.** Stacking and ranked places land
  on top of it. The options seam is the declared home for a pre-game choice and is the wrong one
