...
| **The Pi is CURRENT with `main` (2026-08-05, `c069f5a`)** | unguarded — done by hand and **verified from the artifact, because the exit code proves nothing**. Deployed from a clean detached worktree at `c069f5a`, never the shared checkout (a concurrent session's edits once beat an rsync by seconds). Two rsyncs — `packages/game-logic` to `~/packages/game-logic` as a sibling first — then `npm install && npm run build && npm test` ON the device: **306/309**, the 3 failures the expected `claude-md-counts` `ENOENT` on a `../CLAUDE.md` that does not exist beside a standalone directory. It cleared the **per-game-mastery** catalogue (four new chains + four earn-only titles). **A catalogue deploy has no new route, so the artifact check is different from a code deploy's**: `node -e` on the Pi loaded `@boardwalk/game-logic` and printed `ACHIEVEMENTS.length` = **43** with all six games' mastery chains present, `dist/store/catalog.js` (resolved through the `node_modules/@boardwalk/game-logic` **symlink** the API reads) carries `ttl_tactician/wildcard/patience/silvertongue`, and the new PID started **36 s after** that dist was built, `cwd` in the rsync'd tree. Ledger byte-identical either side of the restart (2 profiles, 12 rows, `integrity_check` ok) — a catalogue change moves no money, which is the point of checking. `sudo -n systemctl restart boardwalk-api` is **passwordless** (a narrow sudoers entry), so the restart needs no interactive prompt. **Earlier lesson still true and worth not relearning**: a `POST /<route>` → 401 through the Funnel is NOT proof a route deployed — auth runs before routing, so `POST /bogusroute` → 401 too; only the dist symbol + the PID-postdates-build check proves it |
