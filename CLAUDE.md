...
|---|---|
| **Live house rules are NOT YET DEPLOYED to the Pi** | ⏳ **OWED — and this is the one row in this table that is a blocker rather than a record.** `setHouseRules` is a NEW FRAME, so the ordering rule binds in its strong form: a deployed client drawing an editable toggle at a referee that has never heard of it gets a refusal on a control the table is showing — the UI that lies, which bit the one-chair blackjack table in PR #91 and was found only in a browser. The reverse is fully inert (an old client never sends the frame). **So the Pi goes FIRST and the client half must not merge until it has.** What to verify from the ARTIFACT rather than the exit code, per every row above: `dist/rooms/gateway.js` naming `setHouseRules` and `dist/rooms/store.js` carrying the writer; the running `MainPID` postdating that build with `cwd` in the rsync'd tree; the suite green ON the device; and the ledger byte-identical either side of the restart. **Price the boot sweep BEFORE restarting** — it voids and refunds every live round, so count unsettled UNO/Liar's-Dice/blackjack rows and their `pot_cents` first, and state the predicted cost. No schema change and no `COLUMN_MIGRATIONS` entry: this adds no column and no table, only a frame. |
