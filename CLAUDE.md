...
| **Offline hardening is NOT deployed** | The code is in the tree and green; the Pi has no `TICKET_SECRET` yet, so production still accepts client-minted nonces on `/settle` — which is the documented fail-open, not a break. **The Pi goes FIRST** (a server-first deploy is safe because the server accepts both until the secret is set; a client-first one would lose every offline result). Verify from the artifact, not the exit code: `/health` must report `tickets: "on"` and `PRAGMA table_info(ticket_devices)` must list the columns. **No rules deploy** — the feature deliberately adds no stored profile field. Procedure in [plans/OFFLINE_HARDENING.md](plans/OFFLINE_HARDENING.md#deploy-order) |
