...
Dice and Blackjack), or a solo game renders its board straight into the shell with no room at all
(Solitaire, now the only one — Blackjack was the other, and its room-less half was DELETED when
two indistinguishable entrances were collapsed into one). Every
...
invisible), the "★ YOUR TURN" cue, and the **move log** — which is not
decoration in a hidden-hand game, because it is the only place the table ever says that a bot drew four
and lost its turn, and which now lives in the table's SIDEBAR under the chat rather than at the
bottom of the board (see `<TableAside>`). The log's one design decision: v1 pushed a formatted SENTENCE over the wire
...
the fan is **5.75rem** a card — ~72% of your own hand; it shipped at 4.5rem, which was perfectly
legible and read as a LEGEND rather than a hand, and an opponent's hand is the second most important
thing on this board — and the ACTIVE CUE moved from a glow on the container to the name — cyan,
...
content-width and centred with a deliberate gap, at FIVE rungs rather than one number, because the
board does not get the viewport — it gets the viewport minus a 20rem sidebar, and the widest table
this game deals is seven seats: at `lg` that column is ~600px and a 7-seat table has to fit in it,
at `2xl` it is ~1100px and the same table must not huddle in the middle of it. **The direction ring is four
arrows and no drawn track.** A faint circle was tried — it does make the concentricity self-evident
— and it puts a second static ring on a felt already carrying a lit draw pile, a lit playable card
and a glowing discard, which is one ring too many; the arrows just needed to be legible, so they are
`text-3xl` in a text token rather than `text-2xl` in a border one. **The active-colour pill is drawn exactly where the CARD cannot say the
colour itself** — over a wild or a +4, and nowhere else. It used to be unconditional and said RED
under a red card beside a red-tinted border, three statements of one fact stacked in the middle of
the table; deleting it and lighting the discard from the active colour instead is right for a
coloured top card and NOT ENOUGH for the case the pill was earning its place in, which is a black
card at the exact moment the colour changed. A soft glow is a fine reminder of something you
already know and a poor announcement of something that just happened. So the condition is a
property of the CARD rather than a timer — a timer shows the answer only to whoever was looking,
and the player who glances up late is the one who needs it — and both complaints are reconciled
rather than traded off.
*Convention only — no guard. `tests/uno-layout.test.ts` pins
...
touches neither a seat nor the bankroll, and since Blackjack's room-less half was deleted it is the
only caller of that seam.** Blackjack was the first (`modes: ['solo']`, board straight into
`<GameShell>`); Solitaire confirmed the same seam carries a game with no economy either, so it reports only `{ outcome: 'win' }` — XP + the
...
**Blackjack is the economy proof, the one game the client does not deal, and it is now a TABLE and
nothing else.** It declares `modes: ['ai', 'online']` and `seats: { min: 1, max: 4 }` — **the only
one-chair table in the registry, and it is a table because the dealer is an opponent who takes no
chair** (`dealerPlays`; see the seat rule under Multiplayer). It read `min: 2` for exactly as long
as it also declared `'solo'`, and collapsing the two entrances is what turned that into a defect:
while the room-less hand existed, "play alone against the dealer" had its own button and the
table's floor of two was merely a second way in — delete it and the only way in is `'ai'`, labelled
"Solo / AI", whose smallest table seated a bot beside you with no way to ask it to leave. Nothing
was broken by the deletion; it removed the other half of a pair, and the manifest still described
the pair. ONLINE still floors at two (`seatRangeFor`), because a table you open for other people
needs a chair for one. It used
to declare `'solo'` as well and carry TWO containers for one rulebook. Both halves worked; the
problem was at the entrance, where the launch modal drew "Play" above "Solo / AI" — two buttons
that both mean "blackjack by myself" and differ only in whether the other chairs hold bots.
`MODE_LABEL` already argues that two buttons a player cannot tell apart is a picker that cannot be
used, and this is that argument reaching the manifest that caused it. **The room-less half was
DELETED rather than hidden behind a mode nothing offers** — `components/Table.tsx`,
`useBlackjackTable`, `BlackjackRepo` and both its implementations went in the same commit, because
the cheapest way to defeat a cutover is to leave the road it replaced standing and an unreachable
board is the same dead reference as a cosmetic with no reader. **The cost, named: blackjack no
longer deals during a Pi outage.** The room-less hand had a local twin that ran the shared reducer
client-side over ordinary `bet`/`settle` intents; a TABLE cannot exist on that path at all, for
UNO's and Liar's Dice's reason. `VITE_API_BLACKJACK` went with it — a kill switch whose destination
no longer exists.
The rulebook —
...
The referee shuffles, deals, and settles from its own cards, so the deck and the hole card are never
sent and `payoutCents` stopped being a thing the client says; `components/TableBoard.tsx` is a
RENDERER that draws a card BACK for the hole card it genuinely does not have. `useBet()` still owns
the chip rack but **no longer commits**: the stake leaves the bankroll inside the referee's own
transaction, and committing here too would deduct it twice. `'solo'` is still a `GameManifest` mode
— Solitaire's, and now only Solitaire's.
...
server-dealt hand — a table only, since its room-less half was deleted), Chess (the hot-seat proof: a full wire-safe rulebook, two humans on
...
- **WHAT YOU BOUGHT CAN BE TAKEN OFF.** ✅ Live — `applyUnequip(profile, kind)` in
  `packages/game-logic`, `useStore().unequip`, and a Take-off button on the store card where a dead
  "Equipped" label used to be. Equipping had no inverse, so every kind was a one-way door: there is
  no "none" row to equip, and the only way out of a felt was buying a different one. The felt is
  where it surfaced because it is the loudest — it draws under all six boards at once, so buying
  one to look at it changes every game permanently — and the shape is identical for the other five
  kinds. The KEY is deleted rather than set to `undefined`, which is load-bearing on both
  persistence paths: the API's `coerceUpsert` and `firebaseProfileRepo.save` each rebuild from what
  is PRESENT, so an absent key clears, where a key carrying `undefined` only happens to clear
  because `JSON.stringify` drops it first. **The avatar is excluded by TYPE** (`UnequippableKind`,
  narrowed by `isUnequippable` — `isPackable`'s shape): `Profile.avatar` is a required string that
  the top bar, the leaderboard row and the profile card render unconditionally, so "no avatar" is
  three surfaces drawing an empty span, and the wrong call does not compile rather than being
  refused at runtime. No confirm and no ownership check — nothing is spent, nothing is lost, and
  Equip puts it straight back.
...
- **The one game that can win money does not deal its own cards.** ✅ Live (Phase D, deployed
  2026-07-18; the room-less seam retired later — see below). `BlackjackTableRepo`
  (`open`/`act`) is the whole seam now: **neither request has a field for a card, an outcome or a
  payout** — absent, not validated — and the projection has no `deck` and carries ONE dealer card
  until the round settles. A ceiling could bound "blackjack, pay me 2.5×"; it could never stop it,
  because "did this player actually win" is not a question you can ask about a number. And **the old
  road is closed**: `checkSettle` refuses `gameId: 'blackjack'` outright (`SERVER_DEALT_GAMES`),
  because leaving `POST /bet` + `POST /settle` open at the 2.5× ceiling would make the whole dealer
  opt-in, and the cheapest way to defeat a cutover is to leave the path it replaced standing.
  **`BlackjackRepo` — the ROOM-LESS hand's seam, `deal`/`move`, with an offline twin — is GONE**, in
  the commit that stopped the game declaring `'solo'`. It was correct and it had no caller, which is
  the one state this repo refuses to leave code in. `VITE_API_BLACKJACK` went with it: a kill switch
  is a road back to somewhere, and there is nowhere for that one to go. **The SERVER half is gone
  too, one deploy later** — `POST /blackjack/deal|move`, `domain/blackjack.ts` and the shared
  `viewOf`/`HandView` — and the gap between the two commits is the rule rather than a delay. **That
  is the cutover order REVERSED: the Pi goes FIRST when a client would otherwise reach for
  something that does not exist yet, and LAST when it is the client that is being taken away.**
  Removing the route in the same commit that stopped calling it would have 404'd any cached page
  still holding the old bundle, mid-hand; removing it after that bundle could no longer be live
  costs nothing. `blackjack_hands` and `wagers.hand_id` STAY — 15 rows of history, and dropping
  them would mean rewriting live tables to reclaim nothing, where the ledger is the money record
  and is untouched either way. The projection that survived is `toPublic`, which was never about
  the container.
- **THE DEALER PEEKS, AND INSURANCE IS THE BET THAT THE PEEK IS ABOUT TO HAPPEN.** ✅ Live
  ...
  was hiding two hundred lines away — where the projection's `dealer` carries one card and
  `toPublic` is the only road to the wire; it staked `bet / 2` in floats, where `insuranceStake` floors integer cents
...
- **THE FOURTH DEALT GAME IS THE ONE THAT WAS ALREADY DEALT, AND ITS STAKE IS THE CHAIR'S RATHER
  ...
  untouched: the room-less game's record still holds rows on the Pi and does not move, though
  nothing writes to it any more. The one
  ...
  The solo hand used to still deal there through its own offline twin; it does not, because there
  is no solo hand — blackjack is simply unavailable during a Pi outage now, which is the price of
  one entrance instead of two.
...
- **THE ENTRANCE SIZES ITSELF OFF ITS OWN BOX, NOT THE WINDOW.** ✅ Live — `<TableSetup>` is an
  `@container` (nested: one query decides whether the create panel and the join panels sit side by
  side, the other whether the create panel's own controls split into two columns), and that is a
  correctness property rather than a flourish. The panel has **two homes of very different widths**
  — a modal capped at 80rem, and `/play/:id` inside the shell's `max-w-[110rem]` — and a viewport
  breakpoint answers the same in both, so `md:grid-cols-2` put two columns inside a 736px modal on
  a 1440px screen. A container query asks the only question that matters, which is "it never learns
  where it is" made mechanical. It shipped as one long column of eight controls: on a 1080p screen
  the create panel ran ~600px past the fold, so choosing a stake or a house rule meant scrolling a
  dialog. Now the controls are the TABLE (how many chairs, who may take one, what the other chairs
  play like) beside the GAME (what a chair costs, what rules it is played under), with Create
  spanning both — measured in Chrome at 1920×1080 and ×950: **zero body overflow** on every game
  and every way in, against ~1400px of content before. Two things are rules rather than taste. The
  columns are two explicit stacks and **not** grid auto-placement, which fills row-wise and would
  put "Who can join" BESIDE "Players" on a wide screen — a pairing that changes with the width is
  worse than either. And the split is conditional on there being something to put on both sides,
  because four of the six games declare neither a stake nor a house rule and a second column
  holding nothing is a panel that looks half-loaded. *The column ARRANGEMENT is convention — no
  guard. `launchWidthFor` is tested and nothing static can see a layout, which is exactly why the
  old one shipped: every class was correct.*
...
- **A PLAYER PREFERENCE IS YOURS AND INSTANT; A HOUSE RULE IS THE TABLE'S AND WAITS.** ✅ Live —
  `manifest.playerPrefs`, `src/system/prefs/`, UNO's auto-draw the first caller. There are now
  THREE kinds of thing a game can be played differently under, and they are told apart by two
  questions — **who is bound by it**, and **when may it change**: an `option` is yours and pre-game
  and lives in the URL; a `houseRule` is the whole table's, stamped at create, enforced by the
  referee; a `playerPref` is yours, changeable at any moment, and lives in `localStorage`.
  **The test for which kind something is: if turning it off could change what the rules permit, or
  what anybody else sees, it is not a preference.** Auto-draw passes cleanly — it fires only where
  `mustDraw` is true, the position in which `applyMove` refuses every play and accepts exactly one
  action, and in which drawing ENDS the turn; the reducer had already collapsed the legal set to
  one, so no decision is being taken from anyone and the table is identical either way. "Peek at
  the next card" would fail it, and so would "play my turn for me". It is deliberately NOT an
  `option`, and the options seam's own stated rule is why: **an option change is a NEW GAME, not a
  mutation of one in flight** (Solitaire re-deals; v1's Chess deferred a difficulty change to the
  next game) — right for a rule about how the game plays, fatal for a preference about how your
  client operates the controls, since nobody wants their hand re-dealt for turning an auto-draw
  off. An option value also lives in the URL, which is a *link*, and a link is a thing you share.
  The one field `HouseRuleSpec` has not got is `default`, and that asymmetry is the rule: every
  house rule is OFF unless a table says otherwise, because an on-by-default one silently retunes
  the game for everyone who does not read the lobby — where a preference describes behaviour the
  game ALREADY HAS, so its default must be what shipped, or it disables a live feature under
  everybody. Storage is a boolean keyed by `(gameId, prefId)` and nothing more: the moment a
  preference needs a shape it has stopped being "how my client operates the controls".
- **A GAME DOES NOT DRAW ITS OWN RULES PANEL, and the two kinds are drawn apart.** ✅ Live —
  `<GameRules manifest tableRules>` (`src/system/game/GameRules.tsx`), rendered by `<Lobby>`'s
  in-room header beside `<ExitGame>`; no game spells it. Both facts it shows were reachable only
  BEFORE the deal: house rules are picked on the setup panel and then summarised in the header as
  a line of labels — which says a rule is on and nothing about what it *does*, because that
  sentence is in the manifest and was only ever rendered on the screen you leave in order to play.
  So "can a +4 answer a +2 at this table?" had no answer anywhere on the page at the moment
  somebody plays one. It is the OS's to draw for `<GameOptions>`'s reason (it renders manifest
  DATA) and it sits in the header for `<GameResult>`'s and `<ExitGame>`'s (a control wanted at a
  specific moment must not be somewhere you scroll a felt to find — this is the fourth time).
  **The two kinds are drawn as different things on purpose**: THIS TABLE is a statement, YOURS is
  a set of toggles, so nobody flips a table-wide rule with the click that flips a private one.
  **The house rules are READ-ONLY here and that is a deploy fact, not a design one** — they are
  write-once on the room, enforced in `boardwalk-api`, so an editable toggle would be a control
  the referee refuses (the UI that lies, twice shipped and twice caught); making them changeable
  mid-table is a new wire frame, a relaxation of that guard, and a Pi deploy that has to land
  first. It renders `null` for a game declaring neither kind — five of the six — because a button
  opening an empty panel is the control-that-cannot-change-the-outcome this OS refuses in
  `tableSizeChoices`, `anteChoices` and `houseRuleChoices` already.
...
- **A GAME DOES NOT DRAW ITS OWN END-OF-ROUND PANEL. It says what happened; the OS says where.** ✅
  ...
  makes that free.
- **LEAVE TABLE = BACK TO THE HUB, AND THERE IS EXACTLY ONE OF THEM.** ✅ Live —
  `<ExitGame onExit={onExit} />` (`src/system/game/ExitGame.tsx`), rendered by `<Lobby>`'s in-room
  header and by the two solo boards that have no lobby to draw one for them. A room game's page
  used to carry TWO ways out that did DIFFERENT things: "Leave table" in the header only dropped
  `?table=`, so it did not leave the game — it dumped you on the game's own create-or-join form, the
  page the launch modal exists to stop anybody meeting — while the control that actually reached the
  boardwalk was a second button at the very BOTTOM of the column, under the seat list, under the
  board, under the chat. So the one that worked was the one you had to scroll a full felt to find,
  which is `<GameResult>`'s defect wearing different clothes and was reported in the same words.
  Getting back to a table's setup screen is the browser's BACK button — `enterTable` pushes a
  history entry precisely so that works — and it is not a thing a page needs to offer twice.
  `leaveTable` is **deleted**, not kept for the 'gone' branch, because the road it replaced standing
  is how a cutover gets defeated. The control is `ghost` rather than `quiet`: `quiet` has no border
  and no glow, which is why an exit sitting at the TOP of the page was still reported as missed.
  Guarded both directions in `tests/game-exit.test.ts`, including the one that is only an ORDERING
  fact — the last `<ExitGame>` in `Lobby.tsx` must come before `<ChatPanel>`, since a second one
  appended at the bottom of the column carries no banned phrase and passes every other case.
- **A GAME'S OWN RUNNING PANEL GOES IN THE TABLE'S SIDEBAR, under the chat.** ✅ Live —
  `<TableAside title>` (`src/system/room/TableAside.tsx`), one caller today: UNO's move log. The log
  was drawn at the bottom of the BOARD, below the player's own fan — under the one element that
  grows — so on any real table it was off the bottom of the screen, and the ~7rem it took came out
  of the felt it exists to comment on. A `moveLog` prop on `<Lobby>` would make the OS carry a slot
  named after one game's feature; a component the GAME renders from inside its own board is
  `<GameResult>`'s shape exactly — the game says what the panel is, the OS says where it goes. It is
  a PORTAL because the scrollback is derived from the projection the board already subscribes to,
  and hoisting that into `<Lobby>` would mean the OS holding a game's state. Outside a lobby it
  renders `null` rather than growing a panel in an arbitrary place. The slot element carries
  `empty:hidden`, which is load-bearing: it is a flex item in a `gap`-ed column, so an empty div
  still costs a visible band of dead space at the five games that contribute no panel.
- **A PANEL IN THE TABLE'S SIDEBAR NEVER GROWS THE PAGE — the COLUMN owns the height, and the
  panels split it.** ✅ Live (`tests/table-sidebar.test.ts`). Each panel used to bound ITSELF, the
  chat at `max-h-[60vh]` and the move log at `max-h-[28rem]`, which reads as careful and bounds
  nothing TOGETHER: 648 + 448 plus two headers ran the column to ~1200px beside a ~730px board, so
  the sidebar set the page height, the page grew a scrollbar the board never asked for, and the move
  log — being second — was the half that ended up off the bottom of the screen. That is
  `<GameResult>`'s and `<ExitGame>`'s fold problem arriving a third time through a different door,
  which is why it is a rule rather than a fix. **The mechanism is `lg:absolute lg:inset-0` inside a
  stretched grid item**, not a cap: a grid item contributes its content to the row's height, so a
  merely capped column still pushes the row open — it just stops pushing at the cap. Out of flow,
  the wrapper's content box is EMPTY, the row is the board's height, and the sidebar is stretched to
  exactly that. The page therefore cannot grow at any content length, rather than growing to a
  limit — and it is what makes the 50/50 real, since `flex-1` divides a definite height and nothing
  else. `<TableAside>` owns the scroll container so a GAME cannot give its panel a height at all;
  what a game hands over is lines. Below `lg` there is no board beside it, so it falls back to a
  viewport bound. `min-h-0` is load-bearing in both, exactly as in `<Modal>`.
- **A LOG FOLLOWS ITS OWN TAIL, BUT ONLY WHILE YOU ARE READING THE TAIL.** ✅ Live — `useTailPin`
  (`src/system/room/useTailPin.ts`), shared by the chat and the move log. Each half alone is a bug:
  a log that does not follow shows you the deal for the rest of the game, and one that follows
  unconditionally yanks you out of scrollback every time anybody does anything — which in UNO is
  about once a second, and was exactly what the move log did. The chat had already solved it, so the
  two are now the same lines of code rather than two slacks that drift. It hands back a CALLBACK REF
  and must be DESTRUCTURED at the call site: a hook returning `{ ref, missed, pinned }` puts a ref
  object and render-time values in one bag, and the React compiler then reads every `tail.missed` in
  the caller's JSX as a possible ref access and errors the component out.
- **A TABLE IS AN OBJECT, SO IT HAS A WIDTH OF ITS OWN — and its chairs sit on the arc, not in a
  wrapped row.** ✅ Live — `src/games/blackjack/seatLayout.ts` (`seatArc`, pure) plus
  `components/Felt.tsx`, guarded by `tests/blackjack-layout.test.ts`. This is UNO's board rebuild
  arriving at the one game that never got it: blackjack drew `state.spots.map` into a
  `flex flex-wrap gap-4`, which is a scoreboard that happens to contain cards, over a `bg-base-200`
  panel that is not a table at all. A blackjack table is one of the most recognisable objects in a
  casino and essentially all of that is four things — the green cloth, the dark rail curving round
  it, the print, and the shoe — so the board draws those, and the chairs ride the curve.
  **It differs from UNO in one deliberate way and the reason is geometric**: UNO seats you at the
  bottom and rotates everyone else around you, which is free on a CIRCLE because rotating a cycle
  preserves the cycle. An arc is not a cycle, so rotating it would put seat 3 left of seat 0 on one
  screen and right of it on another — and blackjack's turn order simply IS seat order, the dealer
  working along the arc. Chairs therefore render in seat order on every screen and YOUR chair is
  MARKED rather than moved. **The felt is bounded and centred** for the reason UNO's seats are
  spaced by rungs rather than by `justify-between`: the first build let it fill its container and
  put a 1520px hairline across a desktop with 250px of cards in the middle, which reads as three
  unrelated groups. *The arc's DEPTH and the table's max width are convention — no guard. Nothing
  static can see a layout, which is exactly why the flat version shipped: every class was correct.*
- **THE ODDS PAINTED ON THE FELT ARE DERIVED FROM THE RULEBOOK, never typed onto it.** ✅ Live —
  `src/games/blackjack/tablePrint.ts`, guarded by `tests/blackjack-print.test.ts`. A real table has
  its terms painted on it, and those three lines are the most consequential sentences on the screen:
  a promise about money, made to somebody about to stake some. Typing "BLACKJACK PAYS 3 TO 2" onto
  the felt would put a string beside `payoutCents` that nothing checks — and
  `tests/blackjack-house-odds.test.ts` already says of those exact constants that they are *"each a
  one-character edit away from moving this number, and none of those edits looks like a money change
  in a diff."* Move the natural to 6:5 and a hardcoded felt advertises 3:2 forever, with the whole
  suite green: it is a string, so `tsc` sees nothing; it is not a raw colour, so `no-raw-palette`
  sees nothing; and the edge harness measures the EDGE, which a wrong sign does not move. So the
  stand value is found by ASKING `dealerShouldHit`, and both ratios by asking what `payoutCents` and
  `insurancePayout` actually return. **A game may state its own terms; it may not author them.**
...
- **ARRIVING AT A TABLE SITS YOU DOWN AT IT, AND A TABLE THAT CAME UP FULL DOES NOT ASK YOU TO
  START IT.** ✅ Live (`tests/auto-seat.test.ts`). One rule with two halves, and it is the entrance
  finishing the job `plannedSeats` started. Choosing a game already meant answering every question
  it has — how many chairs, who is in them, what a chair costs, what rules it is played under — in
  a modal that DREW the answer as a seat preview with Create underneath it. Then it put you on a
  second screen showing the same chairs again, with a button. For an AI or hot-seat table that
  button was a click on a foregone conclusion: `store.create` seats the host and fills the chairs in
  one construction and the hot-seat loop runs before you navigate, so `canStart` was **already true
  before the room had rendered once**. And the other way in was worse — a player joining by code or
  from the browser arrived holding nothing, looking at a list of chairs, one of which they then had
  to identify and click, on a question with exactly one sensible answer. Neither is visible to any
  static tool here: every function was correct, and the FLOW was the thing that was wrong.
  **The readiness predicate is ONE function** (`seatsAreReady`), asked by `TableSetup` of the array
  it just built and by `<Lobby>` of the array the room holds — because two spellings could disagree,
  and the way they would is the worst available: the preview promises a seated table, the create
  delivers a seated table, and the lobby asks for a click anyway. **ONLINE KEEPS ITS BUTTON**, which
  is the same decision `fillForMode` already encodes one door along: its chairs are open on purpose,
  so the moment they fill is a moment somebody ARRIVED rather than a moment the host chose, and that
  beat is the feature. **`autoSeatIndex` refuses three cases**, and the load-bearing one is
  `status !== 'waiting'`: a live table started full, so the only claimable chair in one is an `ai`
  seat — which is a departed player's HAND being driven on, not an invitation — and walking a
  passer-by into it hands them somebody else's cards and, at a betting table, somebody else's stake.
  The retry is the SUBSCRIPTION rather than a loop: a lost race comes back `{ok:false}` and the
  snapshot that made it true re-asks the same predicate against fresher seats, so it takes the next
  chair and stops by construction once you hold one. This finally gives `firstClaimableIndex` — the
  "open before ai" rule written in Phase 5 — the caller it was written for and never had.
  **Two things followed and are rules rather than tidying.** A seated player may MOVE while the
  table waits (claim-then-release, strictly that order: the naive spelling frees your chair first
  and can leave you with nothing, and the whole reason you are moving is that other people are
  picking chairs too). And `SeatList`'s **Leave is playing-only**, because in the lobby a vacated
  chair is instantly the best chair for an arriving player and you are one — auto-seat would put you
  straight back and the button would read as broken. That is not a bug to work around: "give up my
  seat but stay on this page" stopped being a state the moment arriving became sitting down, and the
  way out of a table is `<ExitGame>`, which this file already says there is exactly one of.
- **A HOUSE RULE IS A CREATE-TIME ROOM PARAMETER, not a `manifest.option`.** ✅ Live (slices 1–3 of
  ...
  (`manifest.houseRules`, the `manifest.betting` precedent — a game declares, the OS draws), chosen
  at create and thereafter changeable by the HOST ALONE, landing at the next deal (see the next
  rule — the write-once guard is now **one writer**, not none). What has not moved an inch is the
  thing write-once was protecting: nobody may change what game a table is playing under somebody who
  already sat down, which is the ante's rule with the money removed and the fairness left in. The
  rules are stamped onto `UnoGame` by `deal`, so they live in `uno_matches.state_json`,
...
- **A HOUSE RULE CHANGES AT THE NEXT DEAL, AND THE HOST ALONE CHANGES IT.** ✅ Live
  ([plans/done/LIVE_HOUSE_RULES.md](plans/done/LIVE_HOUSE_RULES.md)) — one host-only frame
  (`setHouseRules`), sanitised through the SAME `sanitizeRules` the create path uses, so there is one
  boundary rather than two that can disagree. **The write-once guard is NARROWED, not deleted**:
  seats, status, state and presence must all still leave the bag alone, and this must be the only
  thing that does not — a stronger statement than "nothing changes them", which was silently also
  asserting that no such writer exists. **The "next round" part is free BY CONSTRUCTION and is
  asserted anyway**: `deal` stamps the resolved rules onto the match, so no room-level write can
  reach a round in flight, and `unoGateway.test.ts` drives the whole window — deal under A, change
  the room to B mid-round, the round still reads A for BOTH clients, deal again, B. "Free by
  construction" is how a defect ships. There is deliberately **no `pendingRules` and no schedule**: a
  second home for the rules is the second source of truth this repo keeps deleting.
  **HOST-ONLY IS THE FAIRNESS CALL, and the reasoning is the invariant rather than the convenience.**
  What write-once protected was *"the game you are playing right now cannot change beneath you"* —
  and that is kept by the stamping, not by refusing the write. Guests never had a vote on the rules
  at create (they consent by SITTING DOWN at a table whose rules are on the listing, in the setup
  panel and in `<GameRules>`), so requiring one here would invent a power in order to defend an
  invariant that is not under threat — and it would stall, because a half-agreed rule has a state to
  be in where a rematch does not. The deal that carries the change **already needs everyone**:
  `<Rematch>` is unanimous, so a player who dislikes it declines the deal or leaves. **The
  announcement is therefore load-bearing rather than polish**, since a rule that lands silently makes
  that vote nominal — and it costs no wire field, because the bag already rides on `RoomMeta` and is
  broadcast to every subscriber. It is NOT the move log (which would make the referee synthesise a
  game event for a room fact, crossing the boundary that keeps the server from knowing what a rule
  means — and would be UNO-only) and NOT chat (uid-pinned; a system message needs a synthetic author,
  a rules change and a hand deploy, for a line of text). **The TIME LABEL is a correctness fix, not
  copy**: during a live round the ROOM's rules and the ROUND's differ for exactly as long as a change
  is pending, and both the header line and the panel read the room — so an unqualified "House rules:
  Stacking" over a round dealt without it is a UI that lies. The OS cannot read the match's rules
  (`state` is `unknown` to it, and must stay so) and does not need to: it distinguishes them by TIME,
  so the header says "Next deal:" while playing and the panel says the round in progress keeps what
  it was dealt with. **The ante deliberately stays write-once** — a rules change alters what game you
  are playing, an ante change alters what it costs and is charged automatically inside the deal's own
  transaction, so money keeps the conservative default. **The RTDB fallback REFUSES rather than
  silently succeeding**, because a toggle that reports success and changes nothing is the same lie
...
- **THE WAY OUT IS COLOURED, and it is a third variant rather than a quieter `danger`.** ✅ Live —
  `Button`'s `exit`: `ghost`'s unlit tube wearing error's colour. It shipped `quiet` and was
  reported as missed on a board it was sitting at the TOP of, which is the tell that "put it above
  the fold" was only half the problem; `ghost` fixed the finding and left it looking like every
  other neutral control. Leaving is not destruction (nothing is lost) and it is not neutral either
  (you land somewhere else, and clicking again does not undo it), so it is its own variant — and a
  variant rather than a `className`, because colour comes from `variant` in this kit and the escape
  hatch is layout only. It does **not** glow: the budget is blue = act, cyan = here, gold = money,
  and none of those is "out".
- **A KIT FIELD DOES NOT INVITE THE BROWSER TO FILL IT — except where a caller asks by name.** ✅
  Live — `<Input>` declares `autoComplete="off"` BEFORE `{...rest}`, so it is a default and not a
  policy. Every field in this app is a field in an application (a chat message, a table code, a
  stake, a display name) and the browser answered all of them with a dropdown of things typed into
  some other field once, on top of the thing being typed into; it was worst in chat, where the
  suggestion list covers the conversation. Sign-in is the one place the feature is the point, and
  `AuthPanel` opts back in by name (`username` / `email` / `current-password`) — which works ONLY
  because of the spread order, so that ordering is asserted rather than commented. `autoCorrect` /
  `autoCapitalize` / `spellCheck` are deliberately untouched: they are a different nuisance with a
  different answer per field, and turning them off here would fix nothing that was reported while
  making the one genuinely prose field worse.
...
- **A modal's WIDTH is one of four rungs, and the body flexes rather than clamping.** ✅ Live —
  `size?: 'sm' | 'md' | 'lg' | 'xl'` → `max-w-md` / `max-w-lg` (the default, so nothing that
  shipped moved) / `max-w-3xl` / `max-w-7xl`, named once in `MODAL_WIDTH`. Rungs and not a free
  `className` width for the reason the kit exists: a per-caller width is how five modals end up
  five sizes. **`xl` is a PANEL rather than a dialog**, and it was added because `lg` could not
  carry one: the launch modal's table setup is eight controls, a seat preview and two ways into
  somebody else's table, and at 48rem three of those stack into a column that runs past the fold —
  a form you scroll, which is the complaint `lg` itself was added to answer, one rung further
  along. Which rung a step opens at is **`launchWidthFor`**, a pure function over the manifest, and
  it is a decision in both directions: too narrow is the form you scroll, too wide is Chess
  hot-seat's setup — a heading and a Create button — in a 1280px box, which reads as a panel that
  failed to load the rest of itself. So a table opens at `xl` only when that game's own panel
  fills two columns, asked through the SAME `setupHasTwoColumns` the panel splits on, so the box
  and its contents cannot come to different conclusions about how much there is to draw. The body
...
- **A MODAL BODY IS A SCROLL CONTAINER, so it reserves room for a glow at its own edges.** ✅ Live.
  `overflow-y-auto` is what lets a tall panel scroll instead of overflowing the box — and it also
  makes the element clip at its PADDING BOX, box-shadows included. Every lit control in this kit is
  a `--shadow-glow-*` tube reaching ~18px, so a body with no top padding shaves the halo off the
  first button against a hard horizontal line: it reads as a button cut in half, not as a clipped
  shadow. That shipped, on the entrance to every game. Nothing static could see it — the classes
  were right, the token resolves, and the same button one row down was perfect — which is the
  `open:grid` failure's shape exactly, and it was found the same way, by looking at it in Chrome.
  The header gives its bottom padding back so no existing modal moves, and keeps it when there is
  no body (the confirm host renders one), which is why that padding is a ternary. `tests/modal.test.ts`
  pins the body's vertical padding as a RUNG rather than an exact value, and both sides of the
  header's ternary.
...
- **Assets are curated into the repo, not dumped.** ✅ `public/felts/` (three tables, P5), `public/cards/` (standard 52 + backs, UNO
  set), `public/chips/`, `public/audio/` — the in-use subset of the Game-Shack trove, not the
  whole thing. A staged asset with no reader is the asset form of the game checklist; bring next
  game's art when that game is built. Licence note lives in `public/audio/CREDITS.md` (mostly CC0;
  `felts/` and `chips/` are curated out of the mixed-provenance trove and say so).
  **`public/chips/` is what that rule looks like when it is broken and then fixed.** Eighteen 16×16
  pixel chips were staged in Phase 4 and NOTHING imported them for four phases — the directory
  could have been empty the whole time and no test, lint rule or typecheck would have said a word,
  which is `loadout.color` in asset form. Blackjack's felt is the first thing in this repo to draw
  a chip; it needs denominations printed on the face at a size that survives a betting circle, so
  the set was REPLACED rather than added to, in the same commit that gave the directory a reader.
  One curated directory holding a used set and an unused set is how the next person picks the
  wrong one.
...
| A blackjack chair is a SEAT ON AN ARC, not a cell in a wrapped row | `tests/blackjack-layout.test.ts` (10) — `seatArc` is why the table stopped rendering as `state.spots.map` into a `flex flex-wrap gap-4`, which is a scoreboard that happens to contain cards (UNO's board rebuild fixed the identical defect one game over). Every way this goes wrong draws a picture that looks completely fine: a chair placed twice is a duplicate player, a chair dropped is a table quietly missing somebody, and neither throws nor fails a typecheck. So: each seat placed exactly ONCE and in SEAT ORDER at every size the game deals (read off the REAL registry — and the order is the property, because unlike UNO this layout must NOT rotate you to the middle: an arc is not a cycle, so rotating it would put seat 3 left of seat 0 on one screen and right of it on another, and blackjack's turn order IS seat order); the curve symmetric, deepest in the MIDDLE (the half-moon — inverting it is the one-character `1 - t*t` → `t*t` edit that renders a perfectly tidy table bending the wrong way) and rising monotonically to each end; **every chair strictly off the flat**, which is the `ARC_SPAN` case — at a span of exactly 1 the outermost chairs land where the drop is zero, so a TWO-chair table, the smallest this game deals, would sit both players level with the dealer with no curve at all; a solo hand at the deepest point; and the degradations (an unloaded snapshot is `[]`, not a throw). Falsified by inverting the curve: three go red |
| Every chip on the felt is art on disk, and a wager breaks down into it correctly | `tests/blackjack-chips.test.ts` (19) — three halves that fail in completely different ways. The ART is the `cards`/`audio`/`felts` rule, and it bit harder here than anywhere: `public/chips/` had been staged since Phase 4 with **no reader at all**, so nothing imported it and nothing could have noticed if the directory had been empty the whole time. Every denomination resolves, each to its OWN file (two tiers sharing one image is a $25 chip drawn as a $100 one — the stack still looks like a stack while the felt states a number the art contradicts), the filename is the value PRINTED on the chip's face, and **no denomination is staged that this table cannot stake**, checked against the manifest's own max × 2 (a doubled maximum hand), which is the asset rule as an assertion. The ARITHMETIC is wrong in the quiet way — a breakdown that does not add up draws chips that are not the bet, and nobody counts chips on a screen: exact reconstruction at every whole-dollar amount and across every wager this table can take, doubled and insured; the FEWEST chips (greedy, largest first); descending runs, never a run of zero; a taller stack rather than a missing image above the ladder; and the sub-dollar remainder stated AS a case, because insurance on a $25 hand is $12.50 and the smallest chip is $1 — which is why `<ChipStack>` takes the amount as a required prop and prints it. The THIRD half is `rackChips` — what a player may pick UP, as against what a wager breaks DOWN into, and a second list rather than a reuse because the two answer different questions: the breakdown must cover every amount the table can hold (a $10 chip exists so $35 is two chips rather than eleven), where a rack button for every tier is a row of nine to read before betting $25. Asserted: bounded by the REAL manifest's `betting.max`, since a button `clampBet` snaps straight back is the control-that-cannot-change-the-outcome this file bans elsewhere; a chip small enough to reach the table minimum; ASCENDING, which is the OPPOSITE of the breakdown's order and is what stops somebody tidying the two lists into one and silently reversing the tray; and a SUBSET of the breakdown tiers, so a click that stages $25 is answered by a $25 chip in the circle rather than by two smaller ones, which would read as the table changing your bet. Falsified two more ways — the rack reversed, and a rack tier the breakdown does not know |
| The odds painted on the felt are the odds the ledger pays | `tests/blackjack-print.test.ts` (8) — the three lines on a blackjack table are a promise about money, made to somebody about to stake some, and typing them onto the felt would put a string next to `payoutCents` that nothing checks. Change the natural to 6:5 and the table keeps advertising 3:2 forever: it is a string, so `tsc` sees nothing; it is not a raw colour, so `no-raw-palette` sees nothing; and `tests/blackjack-house-odds.test.ts` measures the EDGE, which a wrong sign on the felt does not move by a basis point. So `tablePrint()` DERIVES all three from the rulebook, and the assertions are PROPERTIES rather than a second copy of the rule (which would be the vacuous guard this section warns about): each quoted ratio is checked against what `payoutCents`/`insurancePayout` actually MOVE across a sweep of real wagers, and the stand value against `dealerShouldHit` at the boundary in both directions. **The case that closes the real hole is the derivation one** — every property above still passes if `tablePrint` is rewritten as a literal array (the derivers stay correct and simply go unused) and so do the pins (they expect the old strings, which is what the literal says), so each LINE is asserted to contain the numbers the derivers produce. Falsified three ways — the natural moved to 6:5, the dealer taught to hit soft 17, and the print hardcoded against a moved rulebook (which only the derivation case and the pin catch) |
| The referee deals a blackjack TABLE, and each chair's money is its own | `boardwalk-api/tests/blackjackTable.test.ts` (23) — opening a round takes NO money (the difference from every other dealt game, asserted on `bet` ROWS rather than an empty ledger, since seeding a profile writes its signup grant); each chair's stake taken through the LEDGER at its OWN number with its own wager row (v1's clobbering bug as a thing that cannot happen); an unaffordable stake refused with nothing written and the nonce given back; the deal firing only on the LAST chair's bet with a bot charged nothing; a double taken as a SECOND stake with `staked_cents` accumulating; a replay re-serving the persisted round against a DRIFTING rng, because a bet consumes the shuffle and a re-run would deal different cards. **The settle is asserted on a round whose two chairs DISAGREE**, off a searched seed — the first draft let the deal decide, both chairs happened to lose, and paying every chair `spots[0]`'s hand passed it green, which is the leaderboard fixture's lesson in another table. Plus `recordOutcome` once per player, nothing recorded at all for a chair that never bet, `checkSettle` still refusing `blackjack`, and the boot sweep refunding a chair's RUNNING TOTAL rather than an ante. **And the ONE-CHAIR round**, which this dealer refused outright until the manifest learned that a dealer is an opponent: the `< 2` floor was copied from `uno.ts` when this file was written, at a moment when the manifest also said `min: 2` and the two agreed — and the symptom when they stopped agreeing is the clearest form of the UI that lies, since the entrance offers the table, the room is created, and the board then sits on "Opening a round…" for ever while the referee refuses the frame the lobby just sent. Nothing throws and nothing is charged; it simply never deals. Asserted by PLAYING one to a settle rather than by checking `ok`, because the turn order, the peek and the settle all index over `spots` and a table of one is the smallest input any of them ever gets. What is still refused is a round with NO chairs. Falsified six ways — the affordability check removed, every chair paid the first chair's hand, `staked_cents` overwritten instead of accumulated, the projection replaced by the raw table, bots scheduled off the turn, and the one-chair floor put back |
...
| The UNO table names the active colour when the CARD cannot | `tests/uno-centre.test.ts` (7) — RENDERED, not grepped (`tests/modal.test.ts`'s trick): `<TableCentre>` takes plain props, so the real component goes through `renderToStaticMarkup` and what is asserted is the markup a browser is handed. It exists because TWO complaints pull in opposite directions and a later reader will only find one of them — the pill was unconditional and redundant over a red 8, then it was gone and a wild's chosen colour lived nowhere at all. Deleting it satisfies the first and breaks the second; keeping it satisfies the second and IS the first. So: a wild and a +4 name the colour, at every one of the four; a coloured card names nothing; the pill follows the ACTIVE colour and never the card's (a wild's own `color` is `'wild'`, so a pill read off the card renders nothing at all); the halo is drawn in BOTH cases, since it is the felt lit by the card rather than a fallback; and the screen-reader line states the colour unconditionally — it is the floor, it must not acquire the pill's condition, and it is stripped before every other assertion so none of them can pass on it by accident. Falsified five ways — the condition removed, the pill deleted, the pill reading `top.color`, the sr line made conditional, and the halo dropped for coloured cards — each landing on its own case. **Plus where it hangs**, which is a different kind of claim and says so: the pill first sat `-bottom-3.5` INSIDE the discard's own box, straddling the card's bottom edge and cutting across its corner radius — a badge come loose, and the only element on this table that covered a card. There is no DOM here, so "it does not overlap a card" is not assertable and is not pretended; that was MEASURED in Chrome instead (16px below the card bottoms, 8px clear of the deck-count badge, 7px clear of the bottom arrow's box, centred on the PAIR rather than on the discard, ring still concentric — and the first measurement was junk because the ring was sampled mid-spin, so the pass freezes it with `reducedMotion`). What is pinned is the ANCHOR those numbers rest on, `modal.test.ts`'s argument exactly: `top-full` means "below the whole pile row" and `-bottom-*` means "pinned to the card", one token apart, and the wrong one renders perfectly. Falsified by re-pinning it |
...
| Daily streak and store math | `tests/rewards.test.ts` (streak/gap/clock-rewind/cap), `tests/store.test.ts` (27 — afford/own/buy/equip across all three kinds, unique ids + avatar-only unique emoji, every rarity present, earn-only unbuyable at any bankroll + has an unlock line, card back/title equip into the `equipped` map without dropping the other, `equippedTitle`) |
| **Equipped is a state you can LEAVE, and taking a thing off actually clears it** | `tests/store.test.ts` `applyUnequip` block (6) — the store drew a dead "Equipped" label and nothing else, so every kind was a one-way door: there is no "none" row to equip, so the only way out of a felt was buying a different felt. Loudest for the felt, which draws under all six boards at once — buy one to look at it and every game has it for good — and identical for the other five kinds. **What can go wrong is entirely about PERSISTENCE**, which is why the assertions are about the SHAPE of the result and not a boolean: both writers rebuild `equipped` from what is present (the API's `coerceUpsert` reconstructs it key by key, `firebaseProfileRepo.save` writes the whole profile at its own path), so an ABSENT key clears and a key carrying `undefined` only *happens* to clear, because `JSON.stringify` drops it before either sees it. `'felt' in equipped` is the one assertion that can tell those apart. Plus: every other equipped kind untouched, input unmutated, a no-op by IDENTITY when nothing of that kind is worn (`setOptionValue`'s discipline, so a redundant call re-renders nothing), and every kind the catalogue sells swept rather than one — "the store sells it, so it can be taken off" has to be true of the kind added next. **The avatar is excluded by TYPE, not by a branch**: `Profile.avatar` is a required string three surfaces render unconditionally, so `applyUnequip(p, 'avatar')` does not compile, and what is asserted is the `isUnequippable` narrowing the store card's button is gated on |
...
| The room referee arbitrates seats, and a forged uid cannot claim one | `boardwalk-api/tests/rooms.test.ts` (46, the store/seat logic) + `gateway.test.ts` (28, driven over a REAL socket) — handshake auth, host-only gating, monotonic `seq`, owner-only private hands, author-pinned chat, disconnect→seat-release |
...
| A table's STAKE is the room's, chosen once, and a guest sees it before sitting | `tests/room.test.ts` `anteChoices` block — always offers "nothing" FIRST (a `betting` manifest means a game *can* be played for money, never must be; zero is also the default, because money must not leave an account because a control went unnoticed), the rungs inside the declared range (UNO's reproduces v1's NONE/$25/$100/$500/$1K), collapse-to-`[0]` when the range admits no rung (the `tableSizeChoices` rule: one option is a control that cannot change the outcome), garbage collapsed rather than rendered, an ascending non-repeating ladder, and — read off the REAL registry — every offered stake integer cents a fresh account could cover, since a fractional rung dies at `validateBet` and a rung above the opening bankroll is a betting mode nobody can open. Plus the server half in `boardwalk-api/tests/rooms.test.ts`: the ante stamped at create and visible on the snapshot, a hostile stake FLOORED to a non-negative integer before it can reach a ledger row, write-once across seats/status/state/presence (so nobody can raise the stakes on a player who already sat down — v1 pushed a retuned ante to the room on change), and on the browser's poster. And the wire, over a REAL socket in `gateway.test.ts`: the stake reaches a GUEST on their own subscription before they take a chair, and an unsent one reads as `0` rather than `undefined`. **And the rung ladder is now a default rather than the vocabulary** — a host may TYPE a stake, so `parseAnte` is the one place free text becomes cents and every way it can be wrong ends at a ledger. Guarded in `tests/room.test.ts`: the shapes a person types (`$1,000.00`, `25.5`, whitespace), EXACT cents on the values a float multiply gets wrong (`Number('12.10') * 100` is 1209.9999999999998 — the obvious implementation, and it writes a fractional number of cents into a bet `validateBet` then refuses at the moment money moves), a third decimal place REFUSED rather than rounded (the `parseInt` war story on the input side of the same number), `parseFloat('25abc')`-shaped junk refused rather than read as a prefix, both ends of the game's declared range, ZERO refused because "None" is a button and a field that silently means the same thing as a button is two controls for one value, garbage `betting` answered rather than thrown, every error naming what to do (`Input`'s own rule), every rung the ladder offers being typeable so the two controls cannot disagree, and the property the parser exists for — **anything it ACCEPTS is a bet the economy will take**, swept over the REAL registry through `validateBet` itself. Falsified four ways: the un-rounded float multiply, the 3-dp guard removed, the range check dropped, and a rung pushed outside it. **One case here was VACUOUS on its first falsification and is worth the sentence**: it asserted the result equalled `Math.round(Number(dollars) * 100)`, which is a test comparing the rule to a copy of the rule — and it stayed green when the parser was falsified to exactly that expression, because `Math.round` is a correct implementation at two decimal places. The expected cents are literals now |
...
| A table's house rules are stamped at create, bounded, write-once, and on the poster | `boardwalk-api/tests/rooms.test.ts` house-rules block (6) — stamped where the dealer reads them (`rulesOf`), `{}` for a room that does not exist rather than `undefined` (a missing room must never read as a rule somebody's hand is played under), a hostile bag BOUNDED at the one moment it crosses the wire (only literal `true`, ids under 32 chars, capped at 16 — a listing frame's size is not a stranger's to choose), **seats/status/state/presence all leaving them alone** (NARROWED from "write-once" when the host gained one writer — the four ordinary paths must still not touch the bag, which is what stops a table acquiring stacking as a side effect of somebody taking a chair; it matters more than the ante's twin because unlike a raised stake it costs the guest nothing measurable, so nothing would ever surface it), and on the public listing, because "UNO" and "UNO with stacking" are different enough games to change whether a stranger wants the chair |
| A house rule changes at the NEXT DEAL, and only the host changes it | `boardwalk-api/tests/rooms.test.ts` (the two new cases) + `unoGateway.test.ts` (2, over a REAL socket). The store half pins the OTHER side of the narrowing — `setHouseRules` is the one writer, it works while the table is PLAYING, `rulesOf` (what the dealer reads) moves with the snapshot rather than lagging it, and a bag arriving on this frame is bounded exactly as one at create, because a second sanitiser here would leave every create-path assertion green while a browser wrote a kilobyte key into a PUBLIC listing. **The socket half is the one that matters and it is the claim the plan refused to take on trust**: "the next-round part is free by construction" is TRUE — `deal` stamps the match — but nothing else in either suite asks what happens to a LIVE round when the room changes underneath it, and the failure is the whole feature inverted (a change that retunes a game mid-hand is exactly what write-once existed to prevent). So it is driven end to end: deal under `stack`, change the room to `playToLast` mid-round, **both** clients still read the dealt rules, both read the new bag off `RoomMeta` (that broadcast IS the announcement host-only is conditional on), then settle and deal again and the new round carries them. Plus host-only over the wire, refused for a SEATED player rather than a stranger — "not at this table" is refused by a dozen other things, and a member is the one whose request has to be turned down on authority alone. Falsified four ways — the host check removed, the projection re-reading the room instead of the match (the realistic "helpful" defect; the headline case goes red), the sanitiser dropped, and the writer made a no-op — each landing on its own cases |
| The host gets toggles, a guest gets a statement, and neither is told the wrong thing about WHEN | `tests/game-rules.test.ts` (18) — RENDERED, not grepped, so what is asserted is the markup a browser is handed. Everything here draws beautifully when it is wrong. The host's toggle count is preferences PLUS every house rule and the guest's is preferences ALONE — asserted as COUNTS rather than "contains `aria-pressed`", because the guest's panel contains it too (preferences are pressable for everybody) and a substring check passes either way. A guest is never told "your changes apply from the next deal", which is a promise made to somebody who cannot act on it. **The correctness case is the time label**: `live` is the one bit separating "these are the rules" from "these are the rules from now on", the panel reads the ROOM either way, and before a deal the caveat must be ABSENT or the panel is hedging about a table where nothing has happened — it is a STRING, so no other guard in this repo can see it. A dependent rule is drawn DISABLED rather than hidden until its prerequisite is on, read off the REAL manifest and asserted to EXIST so the case cannot pass by testing nothing. Plus the source-text half: the lobby hands the writer to `isHost` ALONE, gated in the same expression that passes it — every rendered case above calls the component directly, so all of them stay green if the lobby passes it unconditionally, and the result is a guest clicking toggles the referee refuses. Falsified two ways — the writer ungated, and the time label collapsed to one sentence |
...
| Arriving at a table seats you, and a table that came up full deals itself | `tests/auto-seat.test.ts` (13) — the two pure predicates the rule is built from, swept over the REAL registry because every failure here is a property of the games this app ships. `autoSeatIndex`: the first OPEN chair before a bot one, and the three refusals — **already playing** (asserted as a PAIR, the chair exists AND is not offered, because that chair is a departed player's hand and the case would otherwise pass on a table with nowhere to sit), already seated (idempotence, so the effect needs no memory of having fired), and a full table. Plus the CONTENTION retry — losing chair 1 answers chair 2 against the snapshot that made it true — and the convergence no unit case can reach: three arrivals fill a 4-seat table one chair each, never the host's, ending `seatsAreReady`. A predicate that returned a claimable chair but not a FRESH one would churn three joiners over seat 1 while 2 and 3 stayed empty, and every individual answer would look right. Then `seatsAreReady`, including the empty array (`[].every` is vacuously true, so an unloaded snapshot would read as a table ready to start). And the composition, which is the whole rule: over every game × every declared way in × every creatable size, a table is ready the moment it is created **iff** its chairs were filled by the create — read through the REAL `fillForMode` rather than a re-spelled ternary, with both answers asserted to actually OCCUR so the sweep cannot pass vacuously. Its source-text half pins that `TableSetup` and `<Lobby>` reach that answer through the same function and that the re-expanded expression is GONE, since there is no DOM here and "one function" is the only form of the claim that is assertable; plus no game spelling `setStatus('playing')` or `autoSeatIndex`, the uniformity `<GameResult>` and `<ExitGame>` already have. Falsified five ways — the status check dropped, the already-seated check dropped, the human clause dropped from `seatsAreReady`, `fillForMode` no longer filling an AI table, and the lobby re-inlining its own readiness expression — each landing on exactly one case |
...
| Seats/ordering/lifecycle are correct | `tests/room.test.ts` — claim (open-before-ai, no-evict), `releaseSeat` fallback, `localSeatIds` ×3 modes, `aiSeatsToDrive` host-only, `seq` strictly-fresh + shuffled-delivery, `teardownPlan` (host clears chat/room, guest doesn't), and `tableSizeChoices`: every size in the range inclusive, **nothing at all when the range holds one size** (a picker with one button is a control that cannot change the outcome — falsified by relaxing `max <= min` to `max < min`), a reversed/zero/fractional/NaN range collapsing rather than rendering a broken picker, and — read against the REAL registry, not a fixture — every size it offers being one the lobby can actually START, since `canStart` needs a full table and a picker offering a size the game refuses is a Start button that never lights up. Plus **every room game has SOMEBODY OPPOSITE**, read off the REAL registry: Tic-Tac-Toe declared `{ min: 1, max: 2 }`, meaning "one human is enough" — true of the GAME and false of the TABLE, since `modes` already carries "you can play this alone". `tableSizeChoices` therefore offered `[1, 2]` and the lobby defaults to `seats.min`, so the default Tic-Tac-Toe table was ONE chair, which `tableIsFull` calls full and `canStart` lights up, on a board whose `seats[1]` is `undefined`. It survived because the seat picker was a small unlabelled row on a page nobody looked at twice; the launch modal puts it at eye level. Stated as a rule over the registry rather than a fix to one manifest, and asserted about the SEATS rather than about `tableIsFull` — a table of one IS full, and it is still not a table. **It said "at least two chairs" until Blackjack, which was the right rule counted the wrong way**: chairs were merely how you counted an opponent, and blackjack's opponent is the DEALER, who plays a hand and takes no chair. So the manifest declares `dealerPlays` (`betting.house`'s shape — a fact only the game can know) and the guard asks for an opponent from either source, with a BIJECTION beside it — exactly the games declaring the flag are the games with a one-chair table — because the flag's only effect is permitting `min: 1`, so declaring it without that is `loadout.color` in a manifest and floor-of-1 without it is Tic-Tac-Toe's bug smuggled back past the case above. A solo game is exempt by construction, never mounting a lobby at all. Falsified three ways — the `1` put back (two red), `dealerPlays` dropped while `min: 1` stays (three red), and the flag declared at a floor of 2 (the bijection alone). And **`seatRangeFor`**, the mode's own view of that range: `seats.min` is a fact about the GAME and "online" is a fact about the TABLE, and a one-chair ONLINE table has no chair for anybody else — it comes up full, `seatsAreReady` is true, and the entrance auto-starts it, so "Play Online" would deal a solo hand at a table nobody can join and the browser would (correctly) never list it. Nothing throws, nothing is mischarged, nothing renders badly. One function read by BOTH the picker and the default, floored at two rather than at `min + 1` (the rule is that somebody can join, and one free chair is what that takes), clamped so a game that cannot reach two is left alone instead of handed `min > max`. `tests/auto-seat.test.ts`'s composition case sweeps through it, which is what caught this in the first place. And `humanCapacity`, the pre-deal sibling of `humanCount` that counts an OPEN chair as a human the table could hold — asking `humanCount` of a PLANNED online table answers "one human, so the house banks this" and locks UNO's bot tier at `sharp` on the strength of a guess that nobody else will ever join |
...
| The line under a stuck hand promises a draw only when one is coming | `tests/uno-stuck-line.test.ts` (4) — one string, and the smallest thing in this slice that can lie. Before the preference existed it was always safe: the board announced a draw it was always about to make. The moment the draw became optional the announcing wording became a claim about the FUTURE that is false for anyone who turned it off — they wait out a beat that never arrives and the table looks hung, which is the UI that lies for the fourth time. It is a pure function precisely so the claim is assertable: there is no DOM here and no way to render UNO's board without a room, so nothing could reach it as JSX, which is why the ternary was lifted out of the markup rather than left where it was written. What is asserted is a PROPERTY and not the copy — exactly one of "the table will do it" and "you do it" holds, over every colour and every debt — because pinning the strings alone goes green on a version with the branches swapped. Plus the position being described honestly: a debt names the NUMBER and no colour (a live stack suspends colour matching entirely), an ordinary stuck hand names the ACTIVE colour and no number. Falsified twice — the branches swapped, and the preference ignored |
| A player's own toggle survives a reload, and cannot be coerced back on | `tests/player-prefs.test.ts` (18) — the THIRD kind of thing a game is played differently under, and the one whose failures are all quiet: nothing here crashes, a preference that reads back wrong just makes the board do what the player asked it not to, which reads as the app ignoring them. **The case the resolver exists for is `Boolean('false') === true`** — `localStorage` hands back strings, so the obvious coercion cannot express "turned off" at all and re-enables auto-draw on every reload, which looks exactly like the toggle not saving; the trap is asserted alongside the fix so it cannot be tidied away. Plus the key: namespaced by GAME (two games sharing `autoDraw` share one switch, which reads as a setting leaking rather than a collision), and — the non-obvious one — **the prefix cannot be a prefix of any other storage key this app writes**, since the store SWEEPS `localStorage` by prefix and the mute flag stores `'true'`/`'false'` too, so a collision would be adopted silently and correctly-looking. The sweep leaves an un-parseable value ABSENT rather than guessing, so a hand-edited `"yes"` cannot out-rank the declared default; storage that is missing or throws degrades to the declared defaults for the session rather than throwing at boot. And the DECLARATION half over the real registry: unique ids, a real label, a hint that is absent or non-blank, and `SHIPPED_DEFAULTS` — a PIN, asserted as a set in both directions, because no test can compute "the behaviour this game already had" and the point is that a preference cannot exist until somebody writes that behaviour down. A `default: false` on auto-draw would silently disable a live feature for everyone who never opens the panel. Falsified four ways — the resolver coercing (4 red), the key losing its game namespace, the sweep guessing at hostile values (3 red), and UNO's default flipped — each landing on its own cases |
| The in-game rules panel says what the table plays, and what is only yours | `tests/game-rules.test.ts` (18) — RENDERED, not grepped (`tests/modal.test.ts`'s trick): the real component goes through `renderToStaticMarkup`, so what is asserted is the markup a browser is handed. Every failure here is silent and draws beautifully. **A rule that is OFF rendering as nothing** is the worst of them — a list that marks only what is enabled reads as a list of everything there is, so "does this table stack?" is answered wrongly by silence, which is worse than no panel because the player believes they checked; so the on/off markers are COUNTED against the declared rules rather than merely found present, since a panel marking everything On contains both strings too. **The hints are the feature**: the lobby header already renders "House rules: Stacking", and what it could never say is what a rule DOES — those sentences live in the manifest and were drawn only on the setup screen, which you leave in order to play. The two kinds are asserted to render as different THINGS (`aria-pressed` appears exactly once per preference and on no house rule), under their own headings, with the two sentences that define the kind — private, and immediate. Plus an absent rules bag reading as no rules (the deploy order: `undefined[id]` would take the header down for every room game), the declared default showing when nothing is stored, and both directions of "nothing to show": a game declaring neither kind renders the empty string — found on the REAL registry and asserted to EXIST, so the case cannot pass vacuously — while every game that declares either gets its button. Its source-text half is `tests/game-result.test.ts`'s sibling: no game may draw one, and the LOBBY must, since a panel nobody mounts satisfies every other case here. Falsified five ways — off-rules filtered out (4 red), hints dropped, `aria-pressed` removed (2 red), the empty-panel guard removed, and the lobby's mount deleted |
| The one modal is four widths wide, its body flexes instead of clamping, and its edges do not SHAVE a glow | `tests/modal.test.ts` (11) — the kit's first test that looks at what a component RENDERS, because everything here is a class string and a class string typechecks however wrong it is. It runs the real `<Modal>` through `renderToStaticMarkup` in Node (no DOM: the effects, which is all `showModal()` is, do not run — the markup does), so the assertions are about what a browser is handed rather than what the source looks like. Every `MODAL_WIDTH` entry resolved against a `--container-*` the theme really declares — the `theme-tokens` family, since Tailwind answers an unmatched one by generating nothing and the box silently keeps its old width — and **exactly ONE `max-w-*` on the box** rather than "the right one is present": a stale width left in the shared BOX list beside the size means CSS source order, not the prop, picks the winner, and it looks perfectly correct in the diff that adds it. Plus the default still being `max-w-lg` (the three existing call sites must not move by a pixel), the rung LIST itself pinned so a fifth is a decision rather than a drift, the body carrying `flex-1`/`min-h-0`/`overflow-y-auto` and **no `max-h-*` in any spelling**, header and footer pinned `shrink-0`, and a childless modal still rendering no body at all (`useConfirm` builds one). **One row here came from the browser and could not have come from anywhere else**: `max-h-full` on the box clamps NOTHING unless the dialog pins its single grid row, because an auto row is sized by its item and `max-height: 100%` then resolves against a height the item itself produced. Every class was correct and every assertion above was green while a tall body rendered a 1463px box hanging 679px off the bottom of an 800px screen — so the two classes are asserted together as one mechanism, `minmax(0,1fr)` and not `1fr` (a fr track keeps a min-content minimum, which is `min-h-0`'s trap one layout system across). Falsified six ways — a stale width in BOX, the body re-clamped at a fresh `70vh`, `lg` re-pointed at a container that does not exist, `min-h-0` dropped, the row left auto, and `1fr` for `minmax(0,1fr)` — each landing on its own case. **A second row came from the browser the same way**: `overflow-y-auto` makes the body a scroll container, and a scroll container clips at its PADDING BOX — the box-shadow of anything against that edge included. The body had `px-6 pb-5` and no `pt` at all, so the launch modal's first button had its whole top halo shaved off against a hard line, which reads as a button cut in half rather than as a clipped shadow (measured: the button's top edge and the scrollport's top edge were the same pixel, headroom 0). Every class was correct, every assertion above was green, and no static tool in this repo can see it — the shadow token resolves, the same button one row down renders perfectly. So the body's vertical padding is asserted as a RUNG (`>= 4`, which clears every visible layer of a `--shadow-glow-*` tube) rather than as `pt-5`, since pinning the exact value would go red on a tidy-up that changed nothing about the property; and the header's bottom padding is asserted on BOTH sides of its ternary, because the gap belongs to whichever element can hold it without clipping and a childless confirm dialog has no body to hold it |
| A table-sidebar panel cannot grow the page, and the two of them share it | `tests/table-sidebar.test.ts` (4) — the SOURCE-TEXT guard, `tests/game-exit.test.ts`'s sibling: there is no DOM here, so "the page does not scroll" is not assertable, but the regression has exactly one spelling — somebody gives a panel a height of its own. So no sidebar panel may name a height in ANY unit (swept over the two OS panels **and** every game's `<TableAside>` content, because the panel is the game's and the rule is the OS's); each panel's ROOT must carry `flex-1` and `min-h-0`; the scroll container must belong to `<TableAside>` rather than to a game; and the column must leave the grid row's flow (`lg:absolute lg:inset-0`) rather than merely capping itself, which is the difference between "cannot grow the page" and "grows to a limit". **COMMENTS ARE STRIPPED BEFORE ANYTHING IS SCANNED** — every file here documents the defect by naming the classes that caused it, so the first run flagged `ChatPanel` and `MoveLog` for the `max-h-[60vh]` and `max-h-[28rem]` in their own war stories, and a guard that goes red on the documentation of the bug it prevents is one somebody deletes. **One assertion was VACUOUS until it was falsified**: it asked whether `min-h-0` appeared anywhere in the source, and dropping it from the `<Card>` left every case green because the scrolling body one element down still carried it — a class on the wrong element IS the defect, so it is scoped to the panel's root now. Falsified five ways — a panel re-bounding itself, a GAME's panel naming a height, `min-h-0` and `flex-1` dropped from a root, and the column put back in flow — each landing on its own case |
| No game draws its own way out, and none is left below the fold | `tests/game-exit.test.ts` (7) — the SOURCE-TEXT guard, `tests/game-result.test.ts`'s sibling and for the same reason: there is no DOM here, so "the exit is above the fold" is not assertable and pretending otherwise would be the vacuous-guard failure. What IS assertable is where a second one comes from — a perfectly reasonable-looking `<Button onClick={onExit}>` somebody adds *because the first one was hard to find*. So: no file under the games/room/game trees spells `onClick={onExit}`, none carries the phrase "Back to the hub" (the literal button that had to be scrolled to), every game DECLARING `solo` draws the control itself (a solo board mounts no `<Lobby>`, so forgetting it is a board with no way off it — keyed on declaring rather than being solo-ONLY, because Blackjack is now both), and `<ExitGame>` may not reach for the router, since the old header button called a `leaveTable` that only edited the query string and that is how "leave" came to mean "stay here and show a form". **The case that catches the actual regression is an ORDERING one**: the LAST `<ExitGame>` in `Lobby.tsx` must come before `<ChatPanel>`. A second copy appended at the bottom of the room column is the OS control and carries no banned phrase, so it passes every other case here — and it is precisely the button that was reported. Falsified four ways — a bottom `<ExitGame>`, a bottom "Back to the hub" button (two go red), a solo board losing its control, and the component reaching for `useSearchParams` — each landing on its own case |
| A kit field cannot be filled by the browser, and sign-in still can | `tests/input.test.ts` (3) — RENDERED, not grepped (`tests/modal.test.ts`'s trick), so what is asserted is the markup a browser is handed. `autoComplete` is `off` on every shape a field comes in, an explicit one WINS, and `AuthPanel` declares one on every field it has. The middle case is the whole design: the default is only safe because the attribute sits BEFORE `{...rest}`, which is invisible in a diff, trivially "tidied" the wrong way round, and would break password-manager fill for everyone while every other test stayed green. The matcher is case-INSENSITIVE deliberately — React 19 emits this attribute in its camelCase spelling, which an HTML parser reads identically and a `/autocomplete=/` matcher reads as absent. Falsified twice: the attribute moved after the spread, and one sign-in opt-in deleted |
...
|---|---|
| **Live house rules ARE DEPLOYED to the Pi** | ✅ **2026-08-11, verified from the artifact, and deployed BEFORE the PR merged.** The ordering rule in its strong form: `setHouseRules` is a NEW FRAME, so a deployed client drawing an editable toggle at an old referee gets a **refusal on a control the table is showing** — the UI that lies, which bit the one-chair blackjack table in PR #91. The reverse is fully inert (an old client never sends it). **The currency check answered cleanly**: per-FILE hashes said the Pi was byte-identical to `origin/main` on BOTH trees (34/34 api, 36/36 shared) before the rsync, which proves in one step that it was behind by exactly this branch and that no concurrent session's work was about to ride along — the shared rsync was therefore a genuine no-op (this slice touches no shared code) and was still run, because the procedure decides that and not a guess. Evidence, none of it an exit code: `src` byte-identical to the branch on both trees after (34/34 api at `8c71471c…`, 36/36 shared UNCHANGED at `7ea478c3…`); `dist/rooms/gateway.js` naming `setHouseRules` twice (dispatch + handler) and `dist/rooms/store.js` carrying the writer **calling `sanitizeRules(raw)`** — the boundary asserted on the artifact rather than the source; `node_modules/@boardwalk/game-logic` resolving to `/home/mogar13/packages/game-logic`; **401/404 on the device**, the 3 the expected `../CLAUDE.md` ENOENT — *named rather than assumed*, by reading the failure text AND `ls`-ing `/home/mogar13/CLAUDE.md` to confirm it genuinely is not there beside a standalone directory; and the mtimes running **src (12:47:40) → dist (13:03:13) → PID (13:05:52)** with `cwd` in the rsync'd tree, `MainPID` 289643 → **309496**. **This frame HAS a route to probe and it was probed**: `node -e` on the device, against the resolved `dist` the referee actually runs, created a table at `{stack:true}`, set it PLAYING, and changed the rules mid-game — `{playToLast:true}` landed on both `rulesOf` (what the dealer reads) and the snapshot (what every client reads), a non-literal `junk: 1` was dropped, and a 33-char id bounded to `{}`. **The boot sweep was priced BEFORE restarting**: 2 unsettled UNO rounds, 1 unsettled blackjack round, every `pot_cents`/`staked_cents` 0 and **0 open wagers**, so the predicted cost was three dead tables and zero cents. The journal says `[uno] voided 2 … refunded 0 cents` and `[blackjack] voided 1 … refunded 0 cents` — the prediction paid in full. Ledger byte-identical either side — 5 profiles, 81 rows, $20,238.50, every per-uid sum unchanged, **no new void rows** because there was nothing to give back, `integrity_check ok` — and `/health` on the Funnel answers `{"ok":true,"db":"up","tickets":"on"}` with CORS 204 for the Pages origin. No schema change and no `COLUMN_MIGRATIONS` entry: this adds a frame, not a column. `sudo systemctl restart` returned **0** and logged `Failed with result 'timeout'` on the STOP in the same breath — the documented lie, in both directions at once. Read `MainPID`. |
| **A $5 stake was VOIDED BY HAND on prod, and no deploy was run** | ✅ **2026-08-09.** Two facts, and the first is the unusual one: **deleting the room-less blackjack hand STRANDED an open wager.** `wagers` row 11 (`hand_id: 6`, $5.00, opened 01:15) belonged to a hand nothing shipped can finish any more, and the boot sweep does not reach it — `sweepAbandonedRounds` covers `blackjack_rounds`, never `blackjack_hands`, checked in the built artifact rather than assumed. So it would have sat open forever. It was voided and refunded in ONE guarded transaction matching `voidRound`'s own shape (`UPDATE ... WHERE settled = 0`, then `appendLedger(uid, 'blackjack', +500, 'void')`), against a **fresh backup taken first**. Read back: 53 → **54 rows**, grand total 2,285,000 → **2,285,500**, that uid 285,000 → **285,500**, open wagers 1 → **0**, hand 6 `settled` 0 → **1**, exactly ONE `void` row, every other uid's sum unchanged, `integrity_check ok`. **The hand was holding 21 against a dealer 20 and was NOT played out** — deciding a result by hand on the prod ledger is the exact thing this architecture exists to make unspellable, and void-and-refund is what the sweep does for every other abandoned game. **NO deploy and NO restart**: per-FILE hashes said the Pi was byte-identical to `origin/main` at `4c69c56` on both trees (**36/36** api, **37/37** shared), because PR #87 touched neither — so a restart would have bought nothing and cost something, as a live blackjack round was in progress on table `D7C3` and the boot sweep voids those. `MainPID` 3842968 unchanged throughout, `/health` `{"ok":true,"db":"up","tickets":"on"}`. The two restarts logged at 15:52 and 15:57 that day were somebody else's manual stop/start cycles, not a crash (`NRestarts: 0`) — the documented `Failed with result 'timeout'` lie on the STOP, again. **Still owed: `POST /blackjack/deal|move`, `domain/blackjack.ts` and the shared `viewOf`/`HandView` have no caller and are still on the box** — left standing on purpose so a cached pre-#87 bundle does not 404, and due for removal on a later deploy. **`blackjack_hands` and `wagers.hand_id` are NOT** — an earlier reading of this row called those 15 rows "dead" and due to go with the code, which was wrong: dropping them means rewriting live tables to reclaim nothing, and the ledger is the money record either way. They stay as history |
| **The bot pace (`AI_DELAY_MS` 900/900/1,100 → one shared 1,500) is DEPLOYED to the Pi** | ✅ **2026-08-08, verified from the artifact — and this one deployed AFTER its merge, which is stated rather than glossed.** The ordering rule exists for changes that make a deployed client lie; this is the one shape where it does not bind at all. `AI_DELAY_MS` is server-only: no wire field, no client that reads it, nothing on screen that claims anything about it, so an old referee simply keeps the old pace and the worst case is bots that are still fast. That is the safest deploy-order failure available, and it is worth naming BECAUSE the rule is otherwise absolute. **The currency check answered in one step, again**: per-FILE hashes said the Pi differed by *exactly* four files — the new `rooms/aiPace.ts` and the three dealers that import it — with `packages/game-logic` byte-identical at 37/37, which proves both that it was behind by this change and that no other session's undeployed work was about to ride along. The shared rsync was therefore a genuine no-op and was still run, because the procedure decides that and not a guess. **The `--omit=optional` trap fired for real this time**: `npm install --omit=optional` builds fine and then `npm test` dies on `rollup/dist/native.js`, which is an OPTIONAL platform binary — a plain `npm install` fixes it, and the failure is at TEST time rather than build time, which is exactly why it survives a deploy that only checks the build. Evidence, none of it an exit code: `src` byte-identical to the branch on both trees after the rsync (36/36 api, 37/37 shared); `dist/rooms/aiPace.js` carrying `AI_DELAY_MS = 1_500`, all three dealers naming `aiPace` and **none of them still holding its own literal** (the point of the change, asserted on the artifact); **426/429 on the device**, the 3 the expected `../CLAUDE.md` ENOENT — *named rather than assumed*, by reading the failure text AND `ls`-ing `/home/mogar13/CLAUDE.md` to confirm it genuinely is not there; and the mtimes running **src (16:43:56) → dist (16:46:39) → PID (16:50:58)** with `cwd` in the rsync'd tree, `MainPID` 3428227 → **3515927**. **This constant has no route to probe**, so the check is the catalogue-deploy's: `node -e` on the device loading the resolved `dist` the referee actually runs and printing the value — **1500**. **The boot sweep was priced BEFORE restarting**: 10 unsettled UNO rounds, every `pot_cents` 0, no open wagers, 0 Liar's Dice and 0 blackjack — so the predicted cost was ten dead tables and zero cents. The journal says `voided 11 abandoned round(s), refunded 0 cents`: **eleven, not ten**, because a table was created in the minutes between the pricing and the restart, which is the honest reading of a live box rather than a discrepancy. The cents were the number that mattered and they were 0. Ledger byte-identical either side — 5 profiles, 34 rows, $20,715.00, every per-uid sum unchanged, **zero** void rows because there was nothing to give back, `integrity_check ok` — and `/health` on the Funnel answers `{"ok":true,"db":"up","tickets":"on"}` with CORS 204 for the Pages origin. `sudo systemctl restart` returned **0** and logged `Failed with result 'timeout'` on the STOP in the same breath: the documented lie, in both directions at once. Read `MainPID` |
| **Blackjack's SEATS (slice 3) is DEPLOYED to the Pi** | ✅ **2026-08-08, verified from the artifact, and deployed BEFORE the PR merged.** The strongest form of the ordering rule so far, because this one is new FRAMES *and* new TABLES: `bjStart`/`bjAction` plus `blackjack_rounds`/`blackjack_players`. A deployed client offering "Solo / AI" on the Blackjack card at an old referee draws a lobby, opens a round nobody can deal, and refuses on a control the table is showing — the UI that lies, again. The reverse is fully inert: an old client has no room mode for this game and never sends either frame. **The currency check answered cleanly for once**: per-FILE hashes said the Pi was byte-identical to `origin/main` on BOTH trees (33/33 api, 36/36 shared), so it was behind by exactly this branch and no other session's undeployed work was about to ride along — the one-step answer a rolled-up digest cannot give. Evidence, none of it an exit code: `src` byte-identical to the branch after the rsync (35/35 api, 37/37 shared); `dist/domain/blackjackTable.js` and `dist/rooms/blackjackDealer.js` present, `dist/rooms/gateway.js` naming `bjStart` and `bjAction`, `dist/db/schema.js` naming both new tables and `dist/server.js` naming `sweepAbandonedRounds`, with the shared `dist/games/blackjack/logic/table.js` carrying `openRound`/`pendingSeats`/`spotPayout`/`AI_WAGER_CENTS` and `node_modules/@boardwalk/game-logic` resolving to `/home/mogar13/packages/game-logic`; **426/429 on the device**, the 3 the expected `../CLAUDE.md` ENOENT — *named rather than assumed*, by reading the failure text AND `ls`-ing the path to confirm the file genuinely is not there beside a standalone directory; and the mtimes running **src (10:41:41) → dist (10:42:37) → PID (10:45:12)** with `cwd` in the rsync'd tree, `MainPID` 3409064 → **3428227**. **The two new TABLES needed no `COLUMN_MIGRATIONS` entry and that is the schema's own rule rather than an oversight** — `CREATE TABLE IF NOT EXISTS` runs on every open and reaches an existing database; it is only a new COLUMN on an OLD table that silently never lands. Read back after the restart: `blackjack_rounds` and `blackjack_players` both present, the latter carrying `round_id,uid,seat,staked_cents`. **The boot sweep was priced BEFORE restarting and cost nothing**: 0 unsettled UNO rounds, 0 unsettled Liar's Dice matches and 0 open wagers, so the journal carries no void lines at all and the ledger carries **zero `void` rows** — the predicted cost, which was zero, paid in full. Ledger byte-identical either side — 5 profiles, 32 rows, $20,705.00, every per-uid sum and xp unchanged, `integrity_check ok` — and `/health` on the Funnel answers `{"ok":true,"db":"up","tickets":"on"}` with CORS 204 for the Pages origin. `sudo systemctl restart` returned 0 **and** logged `Failed with result 'timeout'` on the STOP, both in the same breath: the documented lie, in both directions at once. Read `MainPID` |
| **Blackjack's peek + insurance (slice 1) is DEPLOYED to the Pi** | ✅ **2026-08-08, verified from the artifact, and deployed BEFORE the PR merged.** It had to go first in the strong sense: `insure`/`decline` are new `Move`s, so a deployed client drawing an Insure button at an old referee gets a **409 on a control the table is showing** — a UI that lies, and worse than the usual inert-field case. The reverse direction is benign (an old client never sends them). **The currency check paid for itself again**: per-FILE hashes said the Pi differed from `main` in exactly three files — `rooms/seats.ts`, `protocol.ts`, `store.ts` — which looked like a concurrent session's undeployed work and was not: all three diffs are a COMMENT — a docblock's link to the launch-modal plan, which gained a `done/` segment when that plan closed and the Pi predates the move. (Spelling the old path here made `tests/doc-links.test.ts` fail on this very row, which is that guard's own self-scanning trap doing its job.) A single rolled-up digest would have said "different" and left the question open; the per-file diff answered it in one step and made the rsync safe to run. Evidence, none of it an exit code: `src` byte-identical to the branch on both trees after (69/69 files); `dist/domain/blackjack.js` and `dist/routes/blackjack.js` carry `insure`, `dist/domain/mutations.js` carries `sideNetCents`, and the shared `dist` carries `peek`, `canInsure` and `insurancePaidCents`, with `node_modules/@boardwalk/game-logic` resolving to `/home/mogar13/packages/game-logic`; **395/398 on the device**, the 3 the expected `../CLAUDE.md` ENOENT — *named rather than assumed*, by reading the failure text and confirming the file genuinely is not there beside a standalone directory; and the mtimes run **src → dist (+368s) → PID**, `MainPID` 3288196 → **3409064** started 09:27:31 with `cwd` in the rsync'd tree. **The boot sweep was priced BEFORE restarting**: 3 UNO rounds and 1 Liar's Dice match live, every `pot_cents` 0, so the journal's `voided 3 abandoned round(s), refunded 0 cents` and `voided 1 abandoned match(es), refunded 0 cents` is the predicted cost paid in full, and the ledger carries **zero `void` rows** because there was nothing to give back. Ledger byte-identical either side — 5 profiles, 32 rows, $20,705.00, every per-uid sum unchanged, `integrity_check ok` — and `/health` on the Funnel answers `{"ok":true,"db":"up","tickets":"on"}` with CORS 204 for the Pages origin |
...
| **YOU CAN TAKE IT OFF, SIT ALONE, AND CHANGE YOUR MIND AT THE TABLE** | unguarded by construction — three claims about a browser, and the one that mattered most was invisible to all 1,201 tests and all 400 API tests because both halves were individually correct. Driven 2026-08-11 on the **WS referee** (emulator + a locally-run `boardwalk-api` + Vite), API gate relaxed for the run and restored after. **Zero console errors**; the only 4xx the documented sign-up `404 GET /profile`. **THE STORE**: a felt bought, equipped, and TAKEN OFF — "Equipped" and a Take-off button together, then back to Equip — and the unequip **survived a reload**, which is the only half that can silently fail, since both writers rebuild `equipped` from what is present. The AVATAR section shows its "Equipped" label and **zero** Take-off buttons, which is `isUnequippable` narrowing visible on screen. **BLACKJACK ALONE**: the entrance offers a 1-chair table, it comes up seated with no CPU and no Start button, the felt draws its print and chip rack, and a hand played out against the dealer moved the bankroll. **This is where the drive earned its place** — the first run created the room and then sat on "Opening a round…" for ever, because `openTableRound` refused fewer than two seats: the manifest had learned that a dealer is an opponent and the referee had not. A client change alone would have shipped a table that cannot deal. **ONLINE floors at two**, measured — the picker offers no 1 and does offer 2, so "Play Online" cannot auto-start a solo hand at a table nobody can join. **THE RULES PANEL** renders the table's three house rules with ON/OFF and their explanations, and the auto-draw toggle flips `aria-pressed` true→false, survives a close-and-reopen, and **persists across a reload** (`boardwalk_pref_uno_autoDraw: "false"` still in `localStorage`). One thing the screenshot decided and no assertion could: the toggle was **full-bleed** — a flex column stretches its children — so it read as the dialog's primary action rather than as a switch; it is content-width now (234px in a 512px dialog), matching the lobby's own rule toggles, which are the only other controls of this kind in the app. **One finding is PRE-EXISTING and not this slice's**: a mid-game reload lands on "This table has closed", because the room is reaped while the page is away — reproduced identically on Tic-Tac-Toe, which this branch does not touch |
| **THE BLACKJACK TABLE IS A TABLE YOU CAN BET AT — chips you pick up, a hand that fans, and controls that are ON it** | unguarded by construction — every line is a fact about pixels, and the whole of it was invisible to all 1,154 tests because every class was correct. Driven 2026-08-09 on the **WS referee** (emulator + a locally-run `boardwalk-api` + Vite), API gate relaxed for the run and restored after. **Zero console errors**; the only 4xx the documented sign-up `404 GET /profile`. **The entrance now offers TWO ways in** — the modal's buttons are exactly `SOLO / AI` and `PLAY ONLINE`, where it drew a third ("Play", the room-less hand) that a player could not tell apart from the second. **The rack is chip ART and it is ON the felt**: five buttons resolving to `chip-1/5/25/100/500.png`, zero broken, `allInsideTable: true` measured against the table's own box, and v1's five modifiers (REPEAT ½ 2× ALL IN CLEAR) present — the first probe reported three of them missing and was WRONG, because `innerText` applies `text-transform` and the matcher was case-sensitive, which is the kind of measurement error that reads as a bug. **The FOLD is the number that moved.** The first build measured a **846px** felt with PLACE BET on the last pixel of a 1000px viewport — below the fold on a 1080p screen with browser chrome, which is `<GameResult>`'s and `<ExitGame>`'s defect arriving a third time at the one control a player touches every hand. Rebuilt as two rows it is **703px** with the button **103px clear** at 950px height, and the playing apron's HIT/STAND/DOUBLE **119px clear**, both `allInsideTable`. **The chairs stopped huddling**, which no arithmetic here predicted and one screenshot showed at once: two chairs at the gap ladder sat in a 285px clot in the middle of 1024px of cloth, so the row distributes inside a bounded `max-w-3xl` and a four-chair table now spans **689px** of the 1024, `hOverflow: 0`, no wrap. **The arc is intact and symmetric at four** — nameplate tops 645/658/658/645, the outer pair 13px higher, which is `seatArc`'s own curve. **Two things reserve height that did not**, and both were found the same way: an empty betting circle is a different height from a chip stack, so a chair that had bet and one that had not put their nameplates on different lines and the row read as ragged; and the score bubble only exists once cards are dealt, worth 36px a chair of table jumping at the deal. The remaining settle is 650→709px and is stated as a trade rather than hidden — padding the apron to the tray's full height would put ~70px of dead cloth under the cards for the whole of every hand. **The fan is real**: rotations measured at ±2.5° and ±5°, on their OWN element, because `animate-deal` keyframes `transform` and a rotation sharing that element is replaced mid-deal (the trap UNO's `SeatView` documents and its direction ring hit for real). And the ROUTING change that had no other symptom: `/play/blackjack` with no params — an old bookmark — used to open the room-less board and now lands on the table setup, checked directly |
| **THE BLACKJACK TABLE READS AS A TABLE** | unguarded by construction — every line of this is a fact about pixels, and the defect it closes was invisible to all 1,158 tests because every class was correct. Driven 2026-08-08 on the **WS referee** (emulator + a locally-run `boardwalk-api` + Vite), API gate relaxed for the run and restored after. **Zero console errors on both containers**; the only 4xx is the documented sign-up `404 GET /profile` (a new account's first read before `PUT /profile` creates the row). **The arc is MEASURED rather than argued**: at four chairs the nameplate columns sit at `top` 492 / 504 / 504 / 492 — symmetric, deepest in the middle, the outer pair 12px higher, which is `seatArc`'s own arithmetic (`1.6rem × (1 − 0.72²)` against `× (1 − 0.24²)`) landing on the screen. The table measures **1024 × 585** and is centred: `hOverflow: 0` and `pageScrollPx: 0` at 1600×1000 on both the solo hand and the four-chair table, where the FIRST build put a 1520px hairline across the viewport with the cards huddled in the middle of it — no arithmetic here predicted that and one screenshot showed it at once. Chips resolve to real art and to the right art (`chip-25.png` + `chip-5.png` for a $30 stake, `chip-25` + `chip-10` for $35 — note the $10 chip, which the rack has no button for: the stack breaks down the AMOUNT, not the clicks, which is what a dealer does), **zero broken images**, and the print renders all three derived lines. Both containers were confirmed against the referee rather than the offline twin — `users 2`, one `blackjack_hands` row and one `blackjack_rounds` row in the drive's SQLite — which matters because the first two passes silently ran on the local repo: **`VITE_USE_EMULATOR=1` gates the API off** (`src/system/repo/index.ts`), and the gate's premise is a REMOTE Pi rejecting `demo-boardwalk` tokens, which is not true of an API you started yourself pointed at the emulator. A drive that does not relax it is measuring the wrong path and nothing says so |
| **A BLACKJACK TABLE works, and two people at it stake different numbers** | unguarded by construction — the per-seat half of slice 3 is a claim about two browsers and nothing static can see it. Driven 2026-08-08 on the **WS referee** (emulator + a locally-run `boardwalk-api` + Vite, the recipe in Develop), with the API gate relaxed for the run so the economy and the dealt rounds were the referee's too. **Zero console errors**, and the only 4xx were two `GET /profile` 404s — a brand-new account's first read before `PUT /profile` creates the row, which is the designed sign-up flow rather than a fault. The card opens a modal offering exactly **Play / Solo / AI / Play Online**. **SOLO is untouched**: $5,000 → $4,970 on a $30 stake, one face-down hole card, three cards on the felt. **AI**: no ante picker is drawn at all (the `perSeat` rule, visible), the table came up SEATED with Start lit and no Add-CPU click, and **the bot BET on the referee's own timer** — the phase no other dealt game schedules in — then the round settled at `+$30.00` with the ledger carrying one `-3000` bet, one `+6000` settle, one wager closed by `match_id`, NO row for the bot, and `played 1 / won 1`. **ONLINE, two accounts**: the host's table came up with an OPEN chair and Start DARK (§5.3's decline, visible), the guest joined by the shared-link code path and sat, and then the headline — **Ada staked $30 and Bob $105 at the same table**, with the round staying in betting after the first bet and naming the chair it was waiting for ("Waiting for bjg37376…") before dealing. Both settled independently at their own nets (−$30 / −$105), one stats row each. The stored round holds the deck and BOTH dealer cards while the projection carried one. And the two containers coexist for ONE account without touching: the unfinished solo hand's stake stayed open on `hand_id` while the table's closed on `match_id`, which is exactly what that column split is for. **One thing the drive found that is pre-existing and NOT this slice's**: a uid with no `users` row takes the referee's process down on a FK inside `claimNonce` rather than being refused — reachable through `unoStart` and `ldStart` identically, unreachable in prod (every account gets its row at sign-up), and a local-dev trap the memory already records |
| **The sidebar splits with the board, and the bots stopped rushing** | unguarded by construction — the split is a fact about pixels and the pace is a fact about wall-clock time. Driven 2026-08-08 on the **WS referee** (emulator + a locally-run `boardwalk-api` + Vite), gate relaxed for the run and restored after. **The sidebar's height is now the BOARD's, measured**: `asideH` 764 = `boardH` 764 at four seats and 907 = 907 at seven, with the two panels at 387/353 and 459/425 — the ~50/50 that only a definite height can give, where the capped version measured 139/93 beside a 764px board and left both panels hugging their content. `pageScrollPx` is 0 at four seats and 56 at seven, and the 56 is the BOARD's, not the column's. The move log overflows and scrolls INSIDE itself (`logScrolls: true`, pinned to its tail). **The bot pace is measured off the log's own arrival times**: gaps of 1584/1457/1464/1578/1455ms, median **1464ms**, against `AI_DELAY_MS` 1,500 — where the same probe at the old 900 put six bots round the table in half the time. Plus: the drawn circle is gone and four arrows remain (`drawnCircle: 0`, `ringArrows: 4`), and the exit computes to `oklch(0.68 0.2 18)`, which is `--color-error` and not a near-miss. Zero console errors; the only 4xx the documented sign-up `404 /profile`. **One thing this run cost that is worth writing down**: killing a stray listener BY PORT took down another local session's dev server, because `ss -ltnp` names a PID and not whose it is. Pick a port nobody else is on instead |
| **UNO's TABLE reads as a table, and the way out is above the fold** | unguarded by construction — every line of this is a fact about pixels, and the two defects it closes were both invisible to all 1,097 tests. Driven 2026-08-08 on the **WS referee** (emulator + a locally-run `boardwalk-api` + Vite, the recipe in Develop), with the API gate relaxed for the run and restored after. **THE SPINNING RING WAS WALKING OFF THE PILES**, which is what "the spinny thing and the deck aren't centred" turned out to be: the centring translate and `animate-spin` were on ONE element, so the keyframe's bare `rotate()` replaced the translation and the browser interpolated between them by matrix decomposition — correct at t=0, drifting for the rest of the cycle, and therefore correct in any screenshot taken at the wrong moment. `SeatView` documents the identical trap one file over. Split into two elements, and now MEASURED rather than argued: the four arrows sit at radius **120px each, spread 0**, their centroid **0px** off the piles' centre, clearing the cards' circumscribed radius (101px) by 19px — where the old ring's orbit passed INSIDE that radius, cutting the corners of the thing it orbits. **THE TABLE FITS AT EVERY WIDTH**, checked on the widest table this game deals (7 seats): no horizontal overflow and symmetric margins at 1920/1536/1280/1024 (260/148/116/28px each side). The first pass jumped to the wide gap at `lg` and shoved the left flank off the edge of the felt at 1280 — no arithmetic here predicted it and one screenshot showed it immediately, which is why the rungs are five. Plus: **one** exit button on the page (`← LEAVE TABLE`), it lands on the HUB, and it is visible without scrolling at 1280×800; the move log is its own card under the chat reading "played Yellow 9" / "played Red 3" in title case and in colour; the colour pill is gone and the discard sits in the active colour's light; the chat input reports `autocomplete="off"`; and `useTailPin` proved in the browser — an overflowing log sits at its tail with no control, scrolling to the top releases the pin and draws "LATEST ↓", and sending re-pins. Zero console errors and the only 4xx the documented sign-up `404 /profile` (a new account's first read, before `PUT /profile` creates the row) |
| **THE ENTRANCE FITS ON ONE SCREEN, and the four rungs are the right four** | unguarded by construction — a layout is not assertable here (no DOM in this suite), and pretending otherwise is the vacuous guard the Enforcement note warns about. `launchWidthFor` has a test; where the controls SIT does not, which is exactly why the old shape shipped: every class was correct. Measured in Chrome 2026-08-08 (emulator + Vite, headless, real screenshots read rather than only asserted), on every game and every way in, with **zero console errors and zero 4xx**. The headline is the body's own overflow, which is the number the complaint was about: at 1920×1080 and at 1920×**950** (a 1080p screen less browser chrome) it is **0 on all of them** — UNO online, the fullest panel in the app, is an 812px box where its content used to run ~1400px. Both rungs land where they should: UNO and Liar's Dice online open at `xl` (1280px, controls in two columns at x370/x798 with Create spanning both), Tic-Tac-Toe and Chess at `lg` (768px, a 374px create panel beside the 320px side column) — and the `lg` case is the one that caught a real mistake, because the outer split was first written at `@3xl` and a `lg` box hands the panel only 45rem, so every stakeless game stacked its two panels forever and Tic-Tac-Toe's entrance measured **840px where it is now 508**. The container query is real rather than a silent no-op (`containerType: inline-size`, two of them nested) — the check `theme-tokens` exists for, since an unmatched Tailwind variant emits nothing at all. **What is honestly still true**: at 1440×**820** UNO online overflows by **24px**, which is content that genuinely does not fit and is left rather than shaved to hit a number; and a phone (390×844) scrolls 800px, which is the panel stacked into one readable column with no horizontal blowout (`scrollWidth === clientWidth`), because a phone cannot hold eight controls at once and pretending otherwise would mean hiding some. The second home was driven too — `/play/uno?mode=online` renders the same panel at the shell's full width with the page not scrolling at all |
