...
- **A SIGN-IN THAT SUCCEEDED AND THEN FAILED MUST SAY WHICH HALF FAILED.** ✅ Live —
  `describeAuthFailure` (`src/system/auth/authFailure.ts`), an `authError` on the auth store, and
  the panel `<AuthGate>` draws above the form. Signing in is TWO steps that a player experiences as
  one: Firebase Auth checks the credentials, and then the profile is read from the referee. When
  the second failed, `subscribeToSession` answered by setting `signed-out` and **dropping the
  reason** — so the screen toasted "Welcome back.", rendered the sign-in form again, and said
  nothing. On 2026-08-09 the Pi's database stick left the USB bus, systemd stopped `boardwalk-api`
  (its unit requires that mount), every `GET /profile` answered 502, and that was the entire
  user-visible symptom for three and a half hours. **The sign-out is still correct** —
  signed-in-with-no-profile is not a state anything downstream can use — so what changed is only
  that the reason survives it. Three things are rules rather than copy. The message **leads with
  the password**, because the only theory the silent version leaves available is "I typed it
  wrong", and a player will retype it forever; that one sentence is the fix, and the rest is
  suggestion. It **does not name a cause precisely**: a 502 from the referee and a rules refusal
  from RTDB both arrive as an ordinary `Error`, they are not reliably distinguishable from the
  message, and a confidently wrong diagnosis sends someone to fix the wrong thing — so the
  sentence covers both and `detail` carries the machine's own words, which is `NotConfigured`'s
  precedent one component along. And it is a **separate channel from the form's inline error**: a
  bad password is a `RepoResult` the panel renders in place and means "fix your typing", where
  this means the opposite, and one slot holding both is two contradictory messages in one place.
  `describeAuthFailure` is total by construction — it runs on the path that is already going
  wrong, so a throw there replaces a bad sign-in with a blank page.
...
| An unreachable referee reads as an outage, not as a rejected password | `tests/auth-failure.test.ts` (10) — the other branch a developer cannot reach by clicking, `tests/greeting.test.ts`'s sibling: every developer signing in here has a working referee, so the profile-load REJECTION only ever runs in production, during an outage, for a player with no console open. It ran for 3h28m on 2026-08-09 and the whole visible symptom was a green "Welcome back." toast followed by the sign-in form. **Every assertion is about the PAIR, never the status alone** — `signed-out` on its own is exactly what the bug already returned, so asserting it passes on the defect. So: a dead referee lands `signed-out` **carrying** a failure whose `detail` names the `502`; a working sign-in lands `signed-in` carrying **none** (without which a fix that set the field unconditionally would panic over every good sign-in); a `load` returning `null` — the authoritative "no record" — still SELF-HEALS quietly, because telling a brand-new player the server is down is a lie and the two paths differ only by throw-vs-null; a sign-out clears a previous failure, since a stale outage panel over the form is a claim about a request nobody has made; and the hint names the password, which is the one sentence that stops the retype loop. Plus `describeAuthFailure`'s totality, because it runs on the already-failing path and a throw there turns a bad sign-in into a blank page: an `Error`, a thrown string, an empty message, a **hostile `toString`**, and `null`/`undefined` — which must never render the literal word "undefined" at a player, that being a string someone then greps the codebase for. The async helper yields to the MACROTASK queue rather than counting `await Promise.resolve()`, because the self-heal chain is one link longer than the ordinary one and any fixed count silently under-drains exactly one case and reports `'unknown'`. Falsified three ways — the reason dropped on rejection (the original bug: 3 red), the failure set on success too (2 red), and the `null`/`undefined` guard removed (1 red) |
...
|---|---|
| **THE REFEREE COMES BACK BY ITSELF AFTER THE DB STICK DROPS** | unguarded by construction — nothing in this repo can see a systemd unit on somebody's Pi. `deploy/boardwalk-mount-watchdog.{sh,service,timer}` is version-controlled here and installed there, every minute. **The failure it closes is not a crash**: `boardwalk-api.service` declares `RequiresMountsFor=/mnt/boardwalk-db`, so systemd STOPS it cleanly — SIGTERM, exit 0 — when the stick unmounts, which means `Restart=on-failure` never fires, because nothing failed. On 2026-08-09 the NVR's drive dropped and took the whole USB controller with it (`xhci_hcd 0000:01:00.0: USB bus 2 deregistered`), `sdb1` went too, and the referee stayed down 3h28m; `nvr-watchdog` recovers `/media/recordings` only. **It is PASSIVE about USB on purpose** — that sibling already unbinds and rebinds the controller, and two scripts resetting one controller is strictly worse than one, so this one waits for the device node and mounts. **It runs no `fsck`**: the kernel replays the ext4 journal on mount, which is what actually cleaned this filesystem (`EXT4-fs (sdb1): recovery complete`, `integrity_check ok`, not one row lost), and an unattended repair tool pointed at a half-present USB device turns a recoverable outage into a restore. It refuses to start the API unless the mount carries the real database, because with nothing mounted `/mnt/boardwalk-db` is a bare directory on the root filesystem and starting there is how you get a fresh empty ledger beside a real one. **Falsified both directions on the live box**: the mount unit stopped (systemd stopping the API first, today's exact sequence) recovered **unattended in 15s** off the timer — `UNHEALTHY` → `remounted` → `RECOVERED` in the journal, ledger identical at 42 rows either side, `integrity_check ok`; and with the mount healthy and the API stopped BY HAND it left it alone across two runs and wrote no stamp, which is the `/run` stamp's whole reason — a deploy's `systemctl stop` and a systemd-forced one are identical in unit state, and a watchdog that fought the first would restart the API mid-deploy |
| **THE BACKUPS LEAVE THE STICK THEY PROTECT** | unguarded by construction — a `systemd --user` timer on the workstation, `deploy/boardwalk-backup-pull.{sh,service,timer}`. BACKUP.md has said since 2026-07-18 that "the stick and the Pi fail together" and that off-box is a PULL; what was missing was anything that ran it. **The nightly on-Pi backup writes to `/mnt/boardwalk-db/backups` — a directory on the very stick it exists to survive** — and the off-box copies were last refreshed by hand on 2026-07-18, three weeks stale. **A pull unit already existed here and had been failing every morning since at least 2026-08-03**, always the same way: `ssh: Could not resolve hostname boardwalk-pi.tail1bed2f.ts.net: Temporary failure in name resolution`. `Persistent=true` fires a missed run moments after boot and Tailscale's MagicDNS has not settled by then (this box carries tailscaled's `System DNS config not ideal, /etc/resolv.conf overwritten` health warning) — so it went red into a journal nobody reads, which is a backup that is not happening. The fix is a tailnet **IP** among the candidates, needing no DNS at all, tried in order behind the hostname with a retry window. It **verifies rather than copies**, calling the repo's own `npm run restore:drill` on the newest pulled file rather than reimplementing "is this backup sound", and it **fails loudly** at every exit. Driven for real: 15 files pulled → 23 off-box snapshots, drill PASSED (5 users, 40 ledger rows, $21,485.00). Falsified both directions — the primary host made unresolvable still completes via `mogar13@100.104.118.78`, and no route at all exits 1 with `backup-pull FAILED` rather than reporting success |
