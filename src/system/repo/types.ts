...
/**
 * THE DEALT GAMES' SEAMS LIVE NEXT DOOR — `./dealtGames`, re-exported here so every consumer's
 * import is unchanged.
 *
 * They moved when this file crossed the 800-line ceiling, and the group is a real one: each of them
 * describes a game where the CLIENT IS A RENDERER — no card, no hand, no outcome and no payout on
 * any request, and the authoritative profile back on every reply. What stayed here is the ordinary
...
export interface Repos {
  ...
  /**
   * `null` without the game server. The room-LESS blackjack hand keeps its offline twin above; a
   * TABLE has none, for UNO's reason — a client-side dealer would be one player's browser holding
   * the deck and the hole card for everybody at the table.
   */
  readonly blackjackTable: BlackjackTableRepo | null;
}
